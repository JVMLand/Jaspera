...
export function formatJal(source,{tabSize=2,insertSpaces=true}={}){
 ...
 try{
  const scan=new JALLexer(new antlr4.InputStream(source));scan.removeErrorListeners();const original=new antlr4.CommonTokenStream(scan);original.fill();
  ...
  for(let i=0;i<tokens.length;i++){
   const token=tokens[i],text=token.text,gap=source.slice(previous?previous.stop+1:0,token.start);
   if(token.type===-2){result+=gap+text;previous=token;continue;}
   const closing=text==='}',opening=text==='{'&&token.channel===0;
   if(closing)depth=Math.max(0,depth-1);
   const label=labels.has(token.start)||(invalid&&depth===2&&token.type===JALLexer.ID&&tokens[i+1]?.text===':'&&(/\n/.test(gap)||!previous));
   const level=label?depth-1:depth;
   if(!previous)result+=indent(level);
   else if(previous.type===-2){result+=gap.replace(/[^\r\n]*$/,indent(level));}
   else{
    const oldBreaks=(gap.match(/\n/g)??[]).length;
    const boundary=!invalid&&(starts.has(token.start)||labelEnds.has(previous.stop));
    const newline=oldBreaks>0||previous.type===JALLexer.LINE_COMMENT||(!invalid&&(closing||previous.text==='{'||boundary));
    ...
    else if(['->',':','/', '|','~'].includes(text)||['->',':','/','|','~','[','('].includes(previous.text)||[',',']',')'].includes(text)||token.type===JALLexer.METHOD_DESCRIPTOR&&(previous.type===JALLexer.ID||previous.text==='<init>'||previous.text==='<clinit>'||previous.text.endsWith('|')))result+='';
    else result+=' ';
   }
   result+=text;if(opening)depth++;previous=token;
  }
  return result+eol;
 }catch{return source;}
}
