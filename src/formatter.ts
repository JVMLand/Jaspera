...
import antlr4, { CharStream, type ParserRuleContext, type Token } from 'antlr4';
...
import {
  ...
  InstructionContext,
  LabelContext,
  JvmInsArgLookupSwitchCaseContext,
...
export function formatJal(source: string, { tabSize = 2, insertSpaces = true } = {}) {
  if (source.length > 1024 * 1024) return source;
  const eol = source.includes('\r\n') ? '\r\n' : '\n',
    unit = insertSpaces ? ' '.repeat(Math.max(1, Math.min(16, tabSize))) : '\t';
  try {
    const scan = new JALLexer(new CharStream(source, false));
    ...
    type FormatToken = Pick<Token, 'start' | 'stop' | 'text' | 'type' | 'tokenIndex'> & {
      channel?: number;
    };
    const directives: FormatToken[] = [];
    let offset = 0,
      continued = false;
    const masked = source
      .split(/(?<=\n)/)
    ...
    const { stream, errors, root } = parseJal(masked),
      ...
      labels = new Set(),
      labelEnds = new Set();
    const walk = (node: ParserRuleContext) => {
      if (
        [
          FieldDefinitionContext,
          MethodDefinitionContext,
          ...
          LabelContext,
          JvmInsArgLookupSwitchCaseContext,
        ].some((context) => node instanceof context) &&
        node.start?.tokenIndex >= 0
      )
        starts.add(node.start.start);
      if (node instanceof LabelContext && node.start?.tokenIndex >= 0) {
        labels.add(node.start.start);
        labelEnds.add(node.stop?.stop);
      }
      for (const child of ruleChildren(node)) walk(child);
    };
    walk(root);
    const tokens = [
    ...
    let result = '',
      ...
      depth = 0;
    const indent = (level: number) => unit.repeat(Math.max(0, level));
    const comment = (t: FormatToken) =>
      t.type === JALLexer.LINE_COMMENT || t.type === JALLexer.BLOCK_COMMENT;
