...
export function analyzeSymbols(source) {
  const result = { classes: [], references: [] };
  ...
  try {
    ...
    for (const c of collect(root, 'classDefinition')) {
      ...
      const owner = classNode.getText(),
        superNode = collect(c, 'classPropSuperClass')[0]?.className();
      const interfaces = collect(c, 'classPropInterfaces').flatMap((n) =>
        n.className().map((x) => x.getText()),
      );
      const info = {
        owner,
        ...span(classNode),
      ...
      for (const f of collect(c, 'fieldDefinition'))
      ...
      for (const m of collect(c, 'methodDefinition')) {
        ...
        for (const label of collect(m, 'labelName')) {
      ...
      for (const ref of collect(c, 'jvmInsArgFieldRef'))
      ...
      for (const ref of collect(c, 'jvmInsArgMethodRef'))
      ...
      for (const ref of collect(c, 'fullQualifiedClassName'))
