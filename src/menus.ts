...
export function installMenus(container: HTMLElement, definitions: Menu[]) {
  ...
  const enabled = (i: number) =>
    Array.from(
      panels[i].querySelectorAll<HTMLButtonElement>('button:not(:disabled):not([hidden])'),
    );
  function close(focus = false) {
    if (open < 0) return;
    const old = open;
    panels[old].hidden = true;
    buttons[old].setAttribute('aria-expanded', 'false');
    open = -1;
    if (focus) buttons[old].focus();
  }
  function show(i: number, focus = false, last = false) {
    close();
    open = i;
    panels[i].hidden = false;
    buttons[i].setAttribute('aria-expanded', 'true');
    buttons.forEach((b, n) => (b.tabIndex = n === i ? 0 : -1));
    if (focus) {
      const rows = enabled(i);
  ...
  definitions.forEach((menu, i) => {
    ...
    panel.className = 'menu-popup';
    panel.id = 'popup-' + menu.label.toLowerCase();
    panel.hidden = true;
    ...
    button.onpointerenter = () => {
      if (open >= 0 && open !== i) show(i);
    };
    button.onkeydown = (e) => {
      if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
        e.preventDefault();
        show(i, true, e.key === 'ArrowUp');
      }
    ...
    panel.onkeydown = (e) => {
      const rows = enabled(i),
        at = rows.indexOf(document.activeElement as HTMLButtonElement);
      if (['ArrowDown', 'ArrowUp', 'Home', 'End'].includes(e.key)) {
        ...
        rows[
          e.key === 'Home'
            ...
            : e.key === 'End'
              ? rows.length - 1
              : (at + (e.key === 'ArrowDown' ? 1 : rows.length - 1)) % rows.length
        ]?.focus();
      }
      if (e.key === 'ArrowLeft' || e.key === 'ArrowRight') {
        e.preventDefault();
        show((i + (e.key === 'ArrowRight' ? 1 : buttons.length - 1)) % buttons.length, true);
      }
    };
  ...
  return {
    ...
    hidden(id: string, value: boolean) {
      items.get(id)!.hidden = value;
    },
    disabled(id: string, value: boolean) {
      items.get(id)!.disabled = value;
    },
  };
}
