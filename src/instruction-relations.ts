...
const families = [
  ...
  ['lconst_0', 'lconst_1', 'ldc2_w'],
  ['fconst_0', 'fconst_1', 'fconst_2', 'ldc', 'ldc_w'],
  ['dconst_0', 'dconst_1', 'ldc2_w'],
  ['ldc', 'ldc_w', 'ldc2_w'],
  ...
  ...'ilfda'
    ...
    .map((t) => [
      t + 'load',
      t + 'store',
      ...[0, 1, 2, 3].flatMap((n) => [`${t}load_${n}`, `${t}store_${n}`]),
    ]),
  ...
  ...'ilfd'.split('').map((t) =>
    'ilfdbcs'
      .split('')
      .filter((to) => to !== t && (t === 'i' || !'bcs'.includes(to)))
      .map((to) => `${t}2${to}`),
  ),
];
const companions: Record<string, string[]> = {
  ...
  invokespecial: ['new'],
  aconst_null: ['ifnull', 'ifnonnull'],
  instanceof: ['ifeq', 'ifne'],
  iinc: ['iload', 'istore', 'iadd', 'wide'],
  jsr: ['astore'],
  jsr_w: ['astore'],
  ret: ['astore', 'return', 'wide'],
  astore: ['ret'],
  wide: [
    'iload',
    'lload',
