...
export function renderFrameTransition(frame:FrameTransition){
 ...
 function pair(before:string[],after:string[],locals=false){
  const unchanged=(locals||!frame.terminal)&&before.length===after.length&&before.every((value,index)=>value===after[index]);
  ...
  (unchanged?[before]:[before,after]).forEach((values,side)=>{
   ...
   const col=node('section',undefined,'frame-column'),body=node('div',undefined,'frame-values');col.append(node('h4',unchanged?'（変更無し）':side?(frame.afterLabel??'実行後'):(frame.beforeLabel??'実行前')),body);
   ...
   if(terminal)body.append(node('div',terminal,'frame-terminal'));
    ...
    for(const index of visible){
     const changed=!unchanged&&(locals?(frame.locals?.changed??values.map((_,i)=>i)).includes(index):index>=values.length-(side?frame.produced:frame.consumed));
