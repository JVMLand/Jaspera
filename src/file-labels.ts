...
export function tabLabels(files: readonly { key: string; path: string }[]): Map<string, string> {
  const entries = files.map((file) => {
    ...
    parts[parts.length - 1] = fileLabel(file.path);
    return { ...file, parts, depth: 1 };
  });
  ...
  for (;;) {
    ...
    const collisions = entries.filter(
      (entry) => counts.get(label(entry))! > 1 && entry.depth < entry.parts.length,
    );
    if (!collisions.length) break;
    for (const entry of collisions) entry.depth++;
  }
