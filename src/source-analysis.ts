...
export class SourceAnalysis {
  ...
  private pending = new Map<
    Model,
    { timer: ReturnType<typeof setTimeout>; dispose: monaco.IDisposable }
  >();
      ...
      const hints: monaco.languages.InlayHint[] =
        cached?.version === model.getVersionId()
          ? cached.data.parameters
              .map((item) => ({
                position: model.getPositionAt(item.offset),
                label: `${item.slot}:`,
  ...
  constructor(
    private changed: (model: Model) => void,
    private editable: (model: Model) => boolean,
  ) {}
  ...
  schedule(model: Model) {
    ...
    const timer = setTimeout(() => {
      ...
      const version = model.getVersionId();
      void this.worker
        .call((api) => api.analyze(model.getValue()))
        .then((data) => {
  ...
  dispose() {
    ...
    this.hints.dispose();
    this.hintsChanged.dispose();
    for (const model of this.pending.keys()) this.cancel(model);
    this.worker.dispose();
  }
}

type GutterState = {
  lines: Map<number, SourceOffset[]>;
  breakpoints: Set<number>;
  current?: number;
};
const gutters = new WeakMap<monaco.editor.IStandaloneCodeEditor, GutterState>();
function gutter(view: monaco.editor.IStandaloneCodeEditor) {
  let state = gutters.get(view);
  if (!state) {
    state = { lines: new Map(), breakpoints: new Set() };
    gutters.set(view, state);
  }
  return state;
}
export function showDebugGutter(
  view: monaco.editor.IStandaloneCodeEditor,
  breakpoints: Set<number>,
  current?: number,
) {
  const state = gutter(view);
  state.breakpoints = breakpoints;
  state.current = current;
  renderGutter(view, state);
}
export function showBytecodeOffsets(
  view: monaco.editor.IStandaloneCodeEditor,
  ...
  for (const item of offsets)
    state.lines.set(item.line, [...(state.lines.get(item.line) ?? []), item]);
  renderGutter(view, state);
}
