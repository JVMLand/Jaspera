...
export function installDebugEditor(editor:monaco.editor.IStandaloneCodeEditor,state:()=>DebugState|undefined,toggle:(uri:string,line:number)=>void,uriForClass:(name:string)=>string|undefined){
 ...
 const click=(event:MouseEvent)=>{
  const model=editor.getModel();if(event.button!==0||!model)return;
  // Monaco's gutter overlays can be the event target instead of the rendered span.
  const slot=[...(root?.querySelectorAll<HTMLElement>('.jal-breakpoint-slot')??[])].find(node=>{const r=node.getBoundingClientRect();return event.clientX>=r.left&&event.clientX<r.right&&event.clientY>=r.top&&event.clientY<r.bottom;});
  if(!slot)return;event.preventDefault();event.stopImmediatePropagation();toggle(model.uri.toString(),Number(slot.dataset.line));
 };
 root?.addEventListener('mousedown',click,true);
 const model=editor.onDidChangeModel(()=>{root?.removeEventListener('mousedown',click,true);root=editor.getDomNode();root?.addEventListener('mousedown',click,true);update();});
 ...
 return {update,dispose(){root?.removeEventListener('mousedown',click,true);model.dispose();action.dispose();decorations.clear();}};
}
