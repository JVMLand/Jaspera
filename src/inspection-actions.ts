...
export function publishInspections(model: monaco.editor.ITextModel, items: Inspection[]) {
  ...
  monaco.editor.setModelMarkers(
    ...
    'jal-inspections',
    items.map((i) => ({
      ...range(model, i.start, i.end),
      message: i.message,
      code: i.code,
      source: 'JAL Inspection',
      severity:
...
export function currentInspections(model: monaco.editor.ITextModel) {
  ...
  return entry?.version === model.getVersionId() ? entry.items : [];
}
monaco.languages.registerCodeActionProvider(
  'jal',
  {
    provideCodeActions(model, selection) {
      ...
      const actions: monaco.languages.CodeAction[] = currentInspections(model)
        .filter(
          (i) =>
            i.edits &&
            i.title &&
            monaco.Range.areIntersectingOrTouching(range(model, i.start, i.end), selection),
        )
        .map((i) => ({
          title: i.title!,
          kind: 'quickfix',
          ...
          diagnostics: monaco.editor
            .getModelMarkers({ owner: 'jal-inspections', resource: model.uri })
            .filter(
              (m) =>
                m.code === i.code &&
                m.startLineNumber === model.getPositionAt(i.start).lineNumber &&
                m.startColumn === model.getPositionAt(i.start).column,
            ),
          edit: {
            edits: i.edits!.map((e) => ({
              resource: model.uri,
              versionId: model.getVersionId(),
              textEdit: { range: range(model, e.start, e.end), text: e.text },
            })),
          },
        }));
