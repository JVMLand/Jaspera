...
function openHelp(title: string, content: Node[]) {
  ...
  actions.append(element('button', '閉じる'));
  ...
  return [
      ...
      ? [
          {
            ...
            action: () =>
              openHelp('プロジェクト', [
                element(
                  'p',
                  'プロジェクトは，設定ファイル（.jalprj）とソースをまとめたフォルダーです。File → フォルダーを開くから選んでください。',
                ),
                element(
                  'p',
                  '実行するファイルは，File → プロジェクトのプロパティで変更できます。初期設定は src/Main.jal です。',
                ),
              ]),
          },
        ]
      : []),
    {
      ...
      action: () => {
        ...
        table.setAttribute('aria-label', 'ショートカット一覧');
        const body = element('tbody');
        table.append(body);
        const row = (label: string, ...alternatives: string[][]) => {
          const tr = element('tr'),
            name = element('th', label),
            keys = element('td');
          name.scope = 'row';
          alternatives.forEach((chord, index) => {
            if (index) keys.append(document.createTextNode(' / '));
            const group = element('span');
            group.className = 'help-key-group';
            chord.forEach((key, i) => {
              if (i) group.append(document.createTextNode(' + '));
              group.append(element('kbd', key));
            });
            keys.append(group);
          });
          tr.append(name, keys);
        ...
        const tabs = element('p');
        tabs.append(
          element('kbd', 'Alt'),
          ' を押しながらタブをクリックすると，同じグループのほかのタブを閉じます。',
        );
        const labels = element('p');
        labels.append(
          element('kbd', 'Shift'),
          ' を押しながらラベルをクリックすると，定義または使用箇所へ移動します。',
        );
    ...
    },
    {
      ...
      action: () =>
        openHelp(APP_NAME, [
          ...
          element('p', 'JVM Assembly Language（JAL）のコードを編集・実行できます。'),
        ]),
    },
  ];
}
