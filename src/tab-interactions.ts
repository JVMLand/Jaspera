...
export function paneWindowExit(workspace:string,detach:(key:string)=>void){
 ...
 let active:string|undefined,opened=false;
 const reset=()=>{active=undefined;opened=false;};
 document.addEventListener('dragstart',event=>{
  reset();const node=(event.target as Element)?.closest<HTMLElement>('[data-pane-key]');
  if(!node)return;
  try{const data=JSON.parse(event.dataTransfer?.getData(MIME)??'null');if(data?.workspace===workspace&&data.key===node.dataset.paneKey)active=data.key;}catch{}
 },{signal:controller.signal});
 document.addEventListener('dragleave',event=>{
  if(!active||opened||event.relatedTarget)return;
  const outside=event.clientX<=0||event.clientY<=0||event.clientX>=innerWidth||event.clientY>=innerHeight||!document.elementFromPoint(event.clientX,event.clientY);
  if(!outside)return;
  opened=true;document.querySelectorAll('.dock-drop-target').forEach(n=>n.classList.remove('dock-drop-target'));detach(active);
 },options);
 document.addEventListener('dragend',reset,options);document.addEventListener('drop',reset,options);
 window.addEventListener('pagehide',reset,{signal:controller.signal});
 return {dispose(){reset();controller.abort();}};
}
