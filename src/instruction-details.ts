...
${operation==='rem'?'余りは、商を 0 の方向に切り捨てる考え方で求めます。割られる値が無限大、または割る値が 0 の場合は NaN になります。':'結果はその型の精度に丸められます。NaN を含む演算の結果は NaN です。無限大と符号付きゼロも扱います。'}${operation==='div'?'ゼロで割っても ArithmeticException は発生せず、値に応じて Infinity または NaN になります。':''}`;
 ...
 if(match=op.match(/^([ilfd])const_(m1|[0-5])$/))return section('命令に含まれる定数',`値は命令名に含まれているので、後ろに数値は書きません。定数プールを使わずに ${type} の ${match[2]==='m1'?'-1':match[2]} を積みます。${/[fd]/.test(match[1])&&match[2]==='0'?' この 0 は正のゼロです。':''}`);
 ...
 if(/^ldc/.test(op))return section('読み出せる定数',op==='ldc2_w'?'long / double 定数を定数プールから読み出します。末尾の 2 は値が 2 スロットを使うことを表します。2つの定数を読む命令ではありません。JAL では例えば ldc2_w 10L と書きます。動的定数の場合も型は long または double です。':`int、float、文字列、クラス、メソッド型、メソッドハンドル、カテゴリ1の動的定数を読み出せます。\n\n${op==='ldc_w'?'定数プールの番号を 16 ビットで指定し、大きな定数プールに対応します。':'ldc のバイトコードでは定数プールの番号を 8 ビットで指定します。指定できる番号は 1〜255 です。'} JAL では定数プールの番号を直接書く代わりに、読み出す定数を書きます。`);
 ...
 const calls:Record<string,string>={
  ...
  invokeinterface:'インターフェースで宣言されたインスタンスメソッドを呼びます。参照先のクラスが実装するメソッドが選ばれます。対象はインスタンスメソッドです。',
 ...
 if(/^goto/.test(op))return section('無条件の分岐','同じメソッド内のラベルへ進みます。ループの先頭に戻る場合や、if の片側の処理後にもう片側を飛ばす場合に使います。'+(op==='goto_w'?' 分岐先までの相対オフセットを符号付き 32 ビットで持ち、goto より遠くへ分岐できます。':' バイトコードの相対オフセットは符号付き 16 ビットです。指定できる範囲は −32768〜32767 バイトです。'));
 if(/^jsr/.test(op)||op==='ret')return section('古いサブルーチン命令',
  (op==='ret'?'命令の引数にスロット番号を指定します。そのスロットに保存された returnAddress を戻り先として、同じメソッド内の実行を再開します。スタックからアドレスを取り出す命令ではありません。':`次の命令のアドレスを returnAddress として積み、同じメソッド内のラベルへ分岐します。分岐先への相対オフセットは符号付き ${op==='jsr_w'?'32':'16'} ビットです。`)+
 ...
 const objectDetails:Record<string,string>={
  ...
  instanceof:'参照先を指定した型に代入できるなら 1、できないなら 0 を積みます。null は 0 です。型が合わなくても ClassCastException は発生しません。',
 ...
 if(/^(pop|dup|swap)/.test(op))return section('値の並べ替え',op.startsWith('pop')?'戻り値など、使わない値を捨てるときに使います。'+(op==='pop'?'取り除けるのはカテゴリ1の値1つです。long / double の半分だけを取り除くことはできません。':'カテゴリ1の値2つ、または long / double の値1つを取り除きます。カテゴリ2の値を分割することはできません。'):op==='swap'?'TOP とその下の値を交換します。どちらもカテゴリ1である必要があり、long / double を直接 swap することはできません。':'計算や保存で消費する前に、同じ値をもう一度使えるようコピーを残します。参照を複製してもオブジェクトは複製されません。同じオブジェクトを指す参照が増えます。\n\nlong / double を途中で分割する並べ替えはできません。上の図の各形式で、値のカテゴリと積む順序を確認してください。');
