...
export function scopedEndpoint(target:Worker|MessagePort){
 ...
 const endpoint:Endpoint={
  ...
  addEventListener(type,listener){listeners.set(listener,type);target.addEventListener(type,listener);},
  removeEventListener(type,listener){listeners.delete(listener);target.removeEventListener(type,listener);},
  start:()=>{if('start' in target)target.start();}
 };
 return {endpoint,dispose(){for(const [listener,type] of listeners)target.removeEventListener(type,listener);listeners.clear();if('close' in target)target.close();}};
}
export class WorkerRpc<T>{
 ...
 call<R>(invoke:(remote:Remote<T>)=>Promise<R>,timeout=60_000):Promise<R>{
  ...
  if(!this.worker){
   this.worker=this.factory();this.scope=scopedEndpoint(this.worker);this.remote=wrap<T>(this.scope.endpoint);
   this.worker.onerror=e=>{e.preventDefault();this.stop(e.message||'Worker の処理に失敗しました。');};
 ...
 stop(message='停止しました。'){
  this.scope?.dispose();this.scope=undefined;if(this.worker){this.worker.onerror=null;this.worker.onmessageerror=null;this.worker.terminate();}this.worker=undefined;this.remote=undefined;
  for(const cancel of this.pending)cancel(new Error(message));
 }
 dispose(){this.closed=true;this.stop('Worker は終了しています。');}
}
