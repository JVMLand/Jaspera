...
interface Guide {id:string;title:string;text:string;target:string;trigger?:string;context?:boolean}
const tab=(name:string)=>`[data-panel="${name}"] [role="tab"]`;
const selected=(name:string)=>tab(name)+'[aria-selected="true"]';
const guides:Guide[]=[
 {id:'debug',title:'一命令ずつ実行',text:'矢印のボタンでステップ実行できます。ステップインは呼び出し先へ，ステップアウトは呼び出し元へ進みます。フレームでスタックとローカル変数を確認できます。',target:'.debug-toolbar:not([hidden])[data-state="paused"]',context:true},
 {id:'instructions',title:'命令を調べる',text:'検索欄から命令名や説明を検索できます。エディタで命令をクリックすると，その命令の説明に切り替わります。使用例の命令にマウスを重ねると，スタックの変化を確認できます。',target:selected('instructions'),trigger:tab('instructions'),context:true},
 {id:'graph',title:'命令のつながりを見る',text:'スタック・ローカル変数・分岐のつながりを図で確認できます。上のチェックボックスで線の種類を切り替え，ドラッグで図を移動できます。',target:selected('graph'),trigger:tab('graph'),context:true},
 {id:'problems',title:'エラーを確認する',text:'項目をクリックすると，問題のある行へ移動します。修正候補がある場合は，エディタの電球から適用できます。',target:selected('problems'),trigger:tab('problems'),context:true},
 ...
 {id:'edit',title:'編集と整形',text:'Edit に検索・置換やコードの整形があります。命令を入力すると補完候補が表示され，命令にマウスを重ねるとスタックの変化を確認できます。',target:'#menu-edit',trigger:'#menu-edit',context:true},
 {id:'build',title:'検査と class の書き出し',text:'Build からコードを検査したり，コンパイルした class ファイルを書き出したりできます。',target:'#menu-build',trigger:'#menu-build',context:true},
];
const prefix='jaspera.guide.dismissed.';
let installed=false;
export function resetFeatureGuides(){window.dispatchEvent(new Event('jaspera:reset-guides'));}
export function installFeatureGuides(){
 if(installed)return;installed=true;
 const dismissed=new Set<string>();
 const read=()=>{for(const guide of guides){try{if(localStorage.getItem(prefix+guide.id)==='1')dismissed.add(guide.id);else dismissed.delete(guide.id);}catch{/* Storage may be disabled; retain session choices. */}}};read();
 ...
 function refresh(){
  request=0;
  if(document.querySelector('dialog[open],.menu-popup:not([hidden]),[role="menu"]:not([hidden]):not(.menu-popup)')){hide();return;}
  const options=guides.filter(g=>!dismissed.has(g.id)&&(!g.context||g.id===preferred||['debug','instructions','graph','problems'].includes(g.id)));
  options.sort((a,b)=>Number(b.id===preferred)-Number(a.id===preferred));
  let next:Guide|undefined,anchor:HTMLElement|undefined;
  for(const guide of options){const node=[...document.querySelectorAll<HTMLElement>(guide.target)].find(visible);if(node){next=guide;anchor=node;break;}}
  if(!next||!anchor){hide();return;}if(next===current&&anchor===target)return;
  hide();current=next;target=anchor;const token=epoch;
  title.textContent=next.title;text.textContent=next.text;close.setAttribute('aria-label',next.title+'のガイドを閉じる');bubble.dataset.guide=next.id;bubble.hidden=false;bubble.style.visibility='hidden';
  const position=()=>{void computePosition(anchor!,bubble,{strategy:'fixed',placement:'bottom',middleware:[offset(18),flip({boundary:[],fallbackPlacements:['top']}),shift({boundary:[],padding:12}),arrow({element:tip,padding:16})]}).then(({x,y,placement,middlewareData})=>{
   if(token!==epoch)return;Object.assign(bubble.style,{left:x+'px',top:y+'px',visibility:'visible'});
   const side=placement.split('-')[0],opposite={top:'bottom',bottom:'top',left:'right',right:'left'}[side]!;
   Object.assign(tip.style,{left:'',top:'',right:'',bottom:''});if(middlewareData.arrow?.x!==undefined)tip.style.left=middlewareData.arrow.x+'px';if(middlewareData.arrow?.y!==undefined)tip.style.top=middlewareData.arrow.y+'px';tip.style.setProperty(opposite,'-5px');bubble.dataset.side=side;
  });};
