...
function openHelp(title: string, content: Node[]) {
  ...
  actions.append(element('button', msg('common.close')));
...
export function helpMenuItems() {
  return [
    ...
    { id: 'help-guides', label: msg('help.showGuideBubblesAgain'), action: resetFeatureGuides },
    {
      id: 'help-offline',
      label: msg('help.prepareForOfflineUse'),
      action: () => {
        void import('../offline/offline').then((m) => m.openOfflinePreparation());
      },
    },
    {
      id: 'help-langjal',
    ...
    },
    {
      ...
      action: () => {
        ...
        openHelp(APP_NAME, [
          ...
          element('p', msg('help.editAndRunJVMAssemblyLanguageJALCode')),
          author,
          article,
        ]);
      },
