...
export function installSearchEverywhere(
  ...
  dialog.setAttribute('aria-label', msg('search.searchEverywhere'));
  ...
  const render = () => {
    ...
    for (const [i, target] of rows.entries()) {
      ...
      kind.textContent = {
        file: msg('search.file'),
        class: msg('search.class'),
        method: msg('common.method'),
        field: msg('common.field'),
      }[target.kind];
    ...
    status.textContent = rows.length
      ? msg('search.results', [rows.length, rows.length === 100 ? msg('search.top') : ''])
      : msg('search.noResultsFound');
    highlight();
  };
  async function choose() {
    ...
    status.textContent = msg('search.openingDefinition');
    try {
      ...
      if (current === version)
        status.textContent = error instanceof Error ? error.message : msg('search.couldNotOpen');
    } finally {
      opening = false;
    }
  }
  async function show() {
    ...
    all = [];
    list.replaceChildren();
    status.textContent = msg('search.preparingSearch');
    dialog.showModal();
    input.focus();
    const current = ++version;
    ...
    if (results.some((result) => result.status === 'rejected'))
      status.textContent = msg('search.someSearchSourcesCouldNotBeLoadedCloseAndTry');
  }
