...
export function installInstructionsPanel(
  ...
  const toolbar = el('div', undefined, 'instruction-search'),
  ...
  const count = el('p', '', 'instruction-count'),
  ...
  chooser.hidden = true;
  chooser.append(category, count, list);
  toolbar.append(chooser);
  const body = el('div', undefined, 'instruction-body');
  body.append(detail);
  host.append(toolbar, body);
  search.setAttribute('aria-expanded', 'false');
  const resize = new ResizeObserver(() => {
    chooser.style.maxHeight =
      Math.max(80, Math.min(400, host.clientHeight - toolbar.offsetHeight - 8)) + 'px';
  });
  resize.observe(host);
  ...
  const outside = (e: PointerEvent) => {
    if (!toolbar.contains(e.target as Node)) toggle(false);
  };
  document.addEventListener('pointerdown', outside, true);
  search.onclick = () => toggle(true);
  search.onkeydown = (e) => {
  ...
  let entries: ReturnType<typeof guide>[] = [];
  let selected = 'iadd',
  ...
  function comparison(form: Diagram) {
    ...
    return renderFrameTransition({
      before: form.before,
      after: terminal ? [] : form.after,
      consumed: form.before.length,
      produced: form.after.length,
      tail: !terminal,
      terminal,
      note: form.note,
      locals: form.locals
        ? { before: localValues(form.locals.before), after: localValues(form.locals.after), labels }
        : undefined,
    });
  }
  ...
  function show(op: string) {
    ...
    detail.style.setProperty(
      '--instruction-color',
      `var(--instruction-${instructionHighlightGroup(op)})`,
    );
    markdown?.dispose();
    ...
    for (const anchor of markdown.element.querySelectorAll<HTMLAnchorElement>('a[data-href]')) {
    ...
    if (entry.related.length) {
      ...
      for (const other of entry.related) {
        ...
        anchor.href = '#instruction-' + other;
        anchor.dataset.op = other;
        anchor.textContent = other;
        anchor.title = target.summary;
        anchor.onclick = (e) => {
          e.preventDefault();
          navigate(other);
    ...
    link.href =
      ...
      op
        .replace(/^([ilfd])const_(?:m1|[0-5])$/, '$1const_$1')
        .replace(/_([0-3])$/, '_n')
        .replace(/^([fd])cmp[lg]$/, '$1cmp_op')
        .replace(/^if_([ai])cmp(?:eq|ne|lt|ge|gt|le)$/, 'if_$1cmp_cond')
        .replace(/^if(?:eq|ne|lt|ge|gt|le)$/, 'if_cond');
    link.target = '_blank';
  ...
  function filter() {
    if (!visible) return;
    if (!entries.length) entries = instructionList.map(guide);
    ...
    for (const name of categories) {
      const group = found.filter((e) => e.category === name);
      ...
      for (const entry of group) {
        ...
        b.type = 'button';
        b.dataset.op = entry.op;
        b.style.color = `var(--instruction-${instructionHighlightGroup(entry.op)})`;
        b.textContent = entry.op;
        b.title = entry.title;
        b.onclick = () => {
          show(entry.op);
          toggle(false);
      ...
      folder.append(buttons);
      list.append(folder);
    }
    show(found.some((e) => e.op === selected) ? selected : found[0].op);
  }
  const context = installContextMenu(host, (target) => {
    const op = target.closest<HTMLElement>('[data-op]')?.dataset.op ?? selected,
  ...
  function navigate(op: string) {
    if (!instructionList.includes(op)) return;
    selected = op;
    search.value = '';
    category.value = '';
    filter();
