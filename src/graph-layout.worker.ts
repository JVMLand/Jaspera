...
const api = {
  outline: (source: string) =>
    analyzeSymbols(source).classes.flatMap((c) =>
      c.members
        .filter((m) => m.kind === 'method' && m.hasCode !== false)
        .map((m) => ({ name: m.name + m.descriptor, owner: c.owner })),
    ),
  positionGraphs: (methods: MethodGraph[]) => positionGraphs(methods, elk),
};
export type GraphLayoutApi = typeof api;
expose(api);
