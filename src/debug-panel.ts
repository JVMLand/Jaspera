...
export function installDebugPanel(root: HTMLElement, actions: DebugActions) {
  ...
  toolbar.setAttribute('aria-label', 'デバッグ操作');
  ...
  list.setAttribute('aria-label', '呼び出しスタック');
  ...
  let state: DebugState | undefined,
    selected = 0;
  const waiting =
    'デバッガは待機中です。Run で実行を開始します。ブレークポイントを置くと，その命令で一時停止してフレームを確認できます。';
  const labels = {
    idle: waiting,
    starting: 'デバッグ実行を準備中…',
    running: '実行中',
  ...
  function render() {
    ...
    indicator.textContent = paused ? '停止中' : state.status === 'starting' ? '準備中' : '実行中';
    ...
    if (selected > 0 && frame.callSnapshot) prediction.beforeLabel = '呼び出し時';
      ...
      prediction.terminal = '呼び出し先の実行待ち';
