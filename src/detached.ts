...
const menus=installMenus(el('menus'),[
 ...
 {label:'View',items:[{id:'wrap',label:'折り返し',action:()=>editor.updateOptions({wordWrap:editor.getRawOptions().wordWrap==='on'?'off':'on'})},{id:'theme',label:'テーマ…',action:()=>{el<HTMLSelectElement>('themes').value=workspace.theme;el<HTMLDialogElement>('theme-picker').showModal();}},null,...(['project','console','problems','instructions','graph'] as const).map(name=>({id:'show-'+name,label:name[0].toUpperCase()+name.slice(1),action:()=>bridge?.openPanel(group,name)}))]},
...
window.addEventListener('pagehide',()=>{filePicker.dispose();dropFiles.dispose();instructionClicks.dispose();toolTabs?.dispose();stackHover.dispose();definitionUI.dispose();bridge?.release(group);observer.disconnect();overlays.remove();sourceAnalysis.dispose();editor.dispose();for(const tab of tabs.values()){tab.model.dispose();}});
