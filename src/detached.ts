...
import './style.css';
import './detached.css';
(self as any).MonacoEnvironment={getWorker:()=>new EditorWorker()};registerLanguage(()=>bridge?.completionCatalog()??Promise.resolve({}));
...
function remove(id:string){const tab=tabs.get(id);if(!tab)return;const keys=[...tabs.keys()],at=keys.indexOf(id);tabs.delete(id);clearTimeout(tab.timer);if(active===id){editor.setModel(null);active=undefined;}tab.model.dispose();if(!active){const next=[...tabs.keys()][Math.min(at,tabs.size-1)];if(next)select(next);}renderTabs();updateActions();}
...
window.addEventListener('pagehide',()=>{dropFiles.dispose();instructionClicks.dispose();toolTabs?.dispose();stackHover.dispose();definitionUI.dispose();bridge?.release(group);observer.disconnect();overlays.remove();worker.dispose();editor.dispose();for(const tab of tabs.values()){clearTimeout(tab.timer);tab.model.dispose();}});
