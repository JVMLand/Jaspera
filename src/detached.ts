...
function update(snapshot:EditorSnapshot){
 ...
 if(!tab){
  ...
  const entry=tab;model.onDidChangeContent(()=>{inspect(entry);if(applying)return;const result=bridge?.edit(entry.state.id,entry.state.version,model.getValue());if(result){update(result);graphFocus();}});inspect(tab);renderTabs();
 }
...
function updateActions(){
 const tab=current();document.title=(toolTabs?.active??tab?.state.title??'Editor')+' — '+APP_NAME;menus.hidden('save-project',!workspace.canSave);menus.disabled('close-tab',!tab&&!toolTabs?.active);menus.disabled('close-others',!tab||tabs.size<2);menus.disabled('open-workspace-file',!workspace.files.length);
...
const filePicker=installFilePicker(files=>{void bridge?.openFiles(files,group);});
const debugActions={start:()=>bridge?.debugStart(active),command:(c:import('./debug-protocol').DebugCommand)=>bridge?.debugCommand(c),stop:()=>bridge?.stop(),reveal:(f:import('./debug-protocol').DebugFrame)=>bridge?.debugReveal(f)};
const debugKeys=installDebugKeys(debugActions);
const debugEditor=installDebugEditor(editor,()=>workspace.debug,(uri,line)=>bridge?.toggleBreakpoint(uri,line),name=>workspace.debug?.documents?.[name]);
const menus=installMenus(el('menus'),[
 {label:'File',items:[{id:'open-files',label:'開く…',shortcut:'Ctrl+O',action:filePicker.open},{id:'open-workspace-file',label:'プロジェクト内のファイル…',action:openFile},null,{id:'save-project',label:'保存',shortcut:'Ctrl+S',action:save},{id:'save-file-as',label:'JAL に書き出す…',action:()=>{const tab=current();if(!tab)return;const url=URL.createObjectURL(new Blob([tab.model.getValue()],{type:'text/plain;charset=utf-8'})),a=document.createElement('a');a.href=url;a.download=tab.state.title.replace(/ \(JAL\)$/,'').replace(/\.class$/i,'.jal').split('/').pop()!;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}},null,{id:'close-tab',label:'このタブを閉じる',action:()=>{if(toolTabs?.active)bridge?.closePanel(group,toolTabs.active);else if(active)closeTab(active);}},{id:'close-others',label:'他のタブを閉じる',action:()=>{if(active)closeTab(active,true);}},{id:'close-window',label:'ウィンドウを閉じる',action:()=>window.close()}]},
 {label:'Edit',items:editMenuItems(id=>{if(id==='undo'||id==='redo'){if(active)bridge?.undo(active,id==='redo');}else action(id);})},
 {label:'View',items:[{id:'wrap',label:'折り返し',action:()=>editor.updateOptions({wordWrap:editor.getRawOptions().wordWrap==='on'?'off':'on'})},{id:'theme',label:'テーマ…',action:()=>{el<HTMLSelectElement>('themes').value=workspace.theme;el<HTMLDialogElement>('theme-picker').showModal();}},null,...(['project','console','problems','instructions','graph','debug'] as const).map(name=>({id:'show-'+name,label:name[0].toUpperCase()+name.slice(1),action:()=>bridge?.openPanel(group,name)}))]},
 {label:'Build',items:[{id:'check',label:'検査',action:()=>bridge?.check(active)},{id:'menu-run',label:'実行',shortcut:'Ctrl+Enter',action:run},{id:'download',label:'class に書き出す…',action:()=>void downloadClass()}]},
