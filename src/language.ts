...
const snippets: Record<string, string> = {
  ...
  invokevirtual:
    'invokevirtual ${1:java/io/PrintStream}->${2:println}(${3:Ljava/lang/String;})${4:V}',
  ...
  ldc: 'ldc "${1:Hello, World!}"',
  iinc: 'iinc ${1:1} ${2:1}',
  new: 'new ${1:java/lang/StringBuilder}',
  newarray: 'newarray ${1:I}',
  bipush: 'bipush ${1:10}',
  sipush: 'sipush ${1:1000}',
};
let jdkPromise: Promise<Record<string, Member[]>> | undefined;
const getJdk = () =>
  (jdkPromise ??= import('./generated/jdk.json').then(
    (m) => m.default as Record<string, Member[]>,
  ));
export function registerLanguage(workspaceCatalog: () => Promise<Catalog> = async () => ({})) {
  monaco.languages.register({
    id: 'jal',
    extensions: ['.jal'],
    aliases: ['JAL', 'JVM Assembly Language'],
  });
  ...
  monaco.languages.setMonarchTokensProvider('jal', {
    ...
    tokenizer: {
      root: [
        ...
        [/[a-zA-Z_$][\w$]*(?:\/[\w$]+)+/, 'type'],
        [/<(?:clinit|init)>/, 'function'],
        [/\b(?:0x[\da-fA-F]+|\d+(?:\.\d+)?(?:[eE][+-]?\d+)?[fFdDlL]?)\b/, 'number'],
        [
          /[a-zA-Z_$][\w$]*/,
          {
            cases: {
              ...Object.fromEntries(
                Object.keys(instructionHighlightGroups).map((group) => [
                  '@' + group,
                  'keyword.instruction.' + group.replaceAll('_', '-'),
                ]),
              ),
              '@keywords': 'keyword',
              '@default': 'identifier',
  ...
  monaco.languages.registerCompletionItemProvider('jal', {
    ...
    async provideCompletionItems(model, position) {
      ...
      if (lineStart) {
        ...
        for (const label of language.instructions.filter((i) => i !== 'aload_4'))
          suggestions.push({
            ...
            insertText:
              /^(getstatic|putstatic|getfield|putfield|invokevirtual|invokestatic|invokespecial|invokeinterface|new|anewarray|checkcast|instanceof)$/.test(
                label,
              )
                ? label + ' '
      ...
      for (const match of model.getValue().matchAll(/^\s*([a-zA-Z_$][\w$]*):/gm))
        suggestions.push({
          label: match[1],
      ...
      if (operand) {
        const opcode = operand[1],
          typed = operand[2] ?? '',
          bare = operand[2] === undefined;
        const start = bare ? prefix.search(/\S/) + 1 : prefix.length - typed.length + 1;
        const memberRange = new monaco.Range(
          position.lineNumber,
          start,
          position.lineNumber,
          word.endColumn,
        ...
        candidates.forEach((c, i) =>
          suggestions.push({
            label: c.label,
            kind: kinds[c.kind],
            insertText: (bare ? opcode + ' ' : '') + c.insertText,
            range: memberRange,
            detail: c.detail,
        ...
        const typed = prefix.trim(),
          start = prefix.length - typed.length + 1;
        const wholeRange = new monaco.Range(
          position.lineNumber,
          start,
          position.lineNumber,
          word.endColumn,
        );
        consoleCompletions(typed).forEach((c, i) =>
          suggestions.push({
            label: c.label,
            kind: kinds[c.kind],
  ...
  monaco.editor.defineTheme('jal-night', {
    base: 'vs-dark',
    inherit: true,
    rules: [
      ...instructionColorRules('jal-night'),
      { token: 'keyword.instruction', foreground: '81C7BE' },
      { token: 'keyword', foreground: 'BBA7EB' },
      { token: 'string', foreground: 'DCD29D' },
      { token: 'comment', foreground: '717D8A' },
      { token: 'type', foreground: 'B8CBDF' },
      { token: 'type.identifier', foreground: 'DFA971' },
      { token: 'number', foreground: 'DFA971' },
    ],
    colors: {
      ...inlayHintColors('#91a1b2'),
      'editor.background': '#151a21',
      'editor.foreground': '#d7dce3',
      'editorLineNumber.foreground': '#52606d',
      'editor.lineHighlightBackground': '#1b222c',
      'editor.selectionBackground': '#2a4957',
      'editorCursor.foreground': '#8fddd0',
