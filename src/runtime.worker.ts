...
function notify(action:(sink:Remote<RuntimeEvents>)=>Promise<void>){const sink=events;if(sink)notifications=notifications.then(()=>action(sink)).catch(()=>{});}
...
function flush() {
  ...
  for (const stream of ['stdout','stderr'] as const) {
    const text=buffers[stream];if(text)notify(sink=>sink.output(stream,text));
    buffers[stream] = '';
  }
}
...
async function initialize(heapMiB:number) {
  ...
  initialization = (async () => {
    ...
    os = await makeBovineOS({ runtimeUrl: root.replace(/\/$/, ''), wasmLocation: new URL('bjvm_main.wasm',root).href,
...
const api={async execute(data:RuntimeRequest,port:MessagePort,heapMiB=128):Promise<Compilation|Disassembly|void>{
  ...
  try {
    ...
    if (data.type === 'compile') {
      ...
      if (!/^[\w$/]+$/.test(className) || className.includes('..') || !bytecode) throw new Error('実行するクラスがありません。');
      const classes=data.compilation.classes??[{className,bytecode}];
      const names=new Set<string>();
      if(!classes.length || classes.length>64)throw new Error('実行するクラスの数が不正です。');
      for(const item of classes) {
        if(!/^[\w$]+(?:\/[\w$]+)*$/.test(item.className) || !item.bytecode || names.has(item.className))throw new Error('実行するクラスが不正または重複しています。');
