...
export async function positionGraphs(methods: MethodGraph[], elk: Pick<ELK, 'layout'>) {
  ...
  for (const [index, method] of methods.entries()) {
        ...
        ...(edge.label
          ? {
              labels: [
                {
                  text: edge.label,
                  width: Math.max(...edge.label.split('\n').map((line) => line.length * 6)),
                  height: edge.label.split('\n').length * 14,
                },
              ],
            }
          : {}),
    ...
    if (compound)
      graph.children = [...grouped].map(([id, items]) => ({
        id,
        layoutOptions: {
          'elk.padding': '[top=18,left=18,bottom=18,right=18]',
          'elk.layered.nodePlacement.strategy': 'INTERACTIVE',
        },
        children: items.map(columnNode),
      }));
    ...
    for (const items of grouped.values())
      for (let i = 1; i < items.length; i++) {
        const from = items[i - 1].id,
          to = items[i].id;
        if (!linked.has(JSON.stringify([from, to])))
          graph.edges!.push({ id: 'order:' + from + ':' + to, sources: [from], targets: [to] });
      }
    ...
    if (compound)
      for (const child of result.children ?? [])
        if (child.children)
          straightenGraphEdges({
            ...child,
            edges: routes
              .filter((item) => item.route.container === child.id)
              .map((item) => item.route),
          });
    for (const node of method.nodes) {
      ...
      nodes.push({
        ...
        y: position.y + position.height / 2 + offset,
        width: position.width,
        height: position.height,
      });
    }
    if (!compound && method.nodes.length) {
      const positions = [...geometry.values()],
        x = Math.min(...positions.map((p) => p.x)) - 12,
        y = Math.min(...positions.map((p) => p.y)) - 12;
      blocks.push({
        id: prefix + [...grouped.keys()][0],
        x,
        y: y + offset,
        width: Math.max(...positions.map((p) => p.x + p.width)) - x + 12,
        height: Math.max(...positions.map((p) => p.y + p.height)) - y + 12,
      });
    }
    for (const { route, x: parentX, y: parentY } of routes) {
      if (route.id.startsWith('order:')) continue;
      const container = route.container ? geometry.get(route.container) : undefined,
        x = container?.x ?? parentX,
        y = container?.y ?? parentY;
      const edge = method.edges[Number(route.id.slice(1))],
      ...
      for (const section of route.sections ?? [])
        edges.push({
          ...
          points: [section.startPoint, ...(section.bendPoints ?? []), section.endPoint].map(
            (point) => ({ x: point.x + x, y: point.y + y + offset }),
          ),
          ...(label ? { x: label.x! + label.width! / 2 + x, y: label.y! + y + offset + 10 } : {}),
        });
    }
    const w = Math.max(result.width ?? 0, method.name.length * 7 + 32),
      h = (result.height ?? 0) + 30;
    const methodNodes = nodes.slice(nodeStart),
      methodBlocks = blocks.slice(blockStart);
    simplifyGraphRoutes(
      [
        ...methodNodes.map((n) => ({
          id: n.id,
      ...
      new Map(methodNodes.map((n) => [n.id, prefix + n.block])),
      { x: 0, y: top, width: w, height: h },
    );
    boxes.push({ name: method.name, x: 0, y: top, width: w, height: h });
