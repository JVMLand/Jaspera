...
export function searchTargets(targets:SearchTarget[],query:string){
 const text=normalize(query.trim()),words=text.split(/\s+/).filter(Boolean);
 return targets.map((target,index)=>{
  const label=normalize(target.label),owner=normalize(target.detail);
  // Descriptors mention other types; those matches are less specific than declaration names.
  const name=target.kind==='method'?label.split('(')[0]:target.kind==='field'?label.split(':')[0]:label;
  const qualified=target.kind==='class'||target.kind==='file'?owner:owner+'.'+name;
  const identity=qualified+' '+name,full=owner+'.'+label;
  let score=Infinity;
