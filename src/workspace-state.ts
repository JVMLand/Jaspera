...
export class WorkspaceStateStore {
  ...
  private state:WorkspaceState={tools:{output:[],stdin:'',problems:[]},canSave:false,running:false,status:'',theme:'jal-night',files:[]};
  get value(){return this.state;}
  update(patch:Partial<WorkspaceState>){
    if(Object.entries(patch).every(([key,value])=>this.state[key as keyof WorkspaceState]===value))return;
    this.state={...this.state,...patch};
    if(this.pending)return;
    this.pending=true;
    queueMicrotask(()=>{this.pending=false;for(const listener of this.listeners)listener();});
  }
  updateTools(patch:Partial<ToolState>){this.update({tools:{...this.state.tools,...patch}});}
