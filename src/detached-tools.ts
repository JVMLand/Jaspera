...
export function installDetachedTools(bridge:DetachedBridge|undefined,group:string,onChange:()=>void){
 ...
 const api={
  ...
  update(state?:ToolState,files:DetachedState['files']=[],doc?:GraphDocument){graph.update(doc);const fileKey=JSON.stringify(files);if(fileKey!==lastFiles){lastFiles=fileKey;renderProjectTree(get('file-list'),files.map(file=>({path:file.title,key:file.key,open:()=>{const state=bridge?.openTab(group,file.key);if(state)bridge?.openDefinition(group,state.uri,{lineNumber:1,column:1});}})),bridge?.workspaceId??'',collapsed,bridge?{create:path=>bridge.projectAction('create',path,true),rename:(path,folder)=>bridge.projectAction('rename',path,folder),move:(path,folder)=>bridge.projectAction('move',path,folder)}:undefined);}if(!state)return;const serialized=JSON.stringify(state);if(serialized===last)return;last=serialized;
