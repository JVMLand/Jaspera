...
export function installDetachedTools(bridge:DetachedBridge|undefined,group:string,onChange:()=>void){
 ...
 const api={
  ...
  dispose(){for(const context of contexts)context.dispose();instructions.dispose();graph.dispose();container.remove();}
 };return api;
}
