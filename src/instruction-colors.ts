...
const membership = new Map(
  Object.entries(groups).flatMap(([group, ops]) =>
    ops.map((op) => [op, group as InstructionGroup] as const),
  ),
);
export function instructionHighlightGroup(op: string) {
  return membership.get(op);
}
...
const accents: Record<string, string> = {
  ...
  'vs-dark': '9CDCFE',
  vs: '005FB8',
  'tsukuba-light': '6600CC',
  'tsukuba-night': 'B395D2',
  'japan-light': '193C87',
  'hitachi-dark': '8EC2D6',
  'hitachi-light': '235A79',
  'denden-night': '78BAFF',
  'denden-light': '0757AD',
  'vibe-night': 'C5A0FF',
  'vibe-light': '713BD0',
  'googol-night': 'A8C7FA',
  'googol-light': '1967D2',
  'entrance-night': '6CB8F6',
  'entrance-light': '006CBE',
};
export function instructionColors(theme: string): Record<InstructionGroup, string> {
  ...
  return Object.fromEntries(
    groupNames.map((g, i) => {
      ...
      if (!accent) return [g, color];
      const tint = [0, 2, 4]
        .map((n) =>
          Math.round(
            parseInt(color.slice(n, n + 2), 16) * 0.9 + parseInt(accent.slice(n, n + 2), 16) * 0.1,
          )
            .toString(16)
            .padStart(2, '0'),
        )
...
export function instructionColorRules(theme: string) {
  const colors = instructionColors(theme);
  return groupNames.map((group) => ({
    token: 'keyword.instruction.' + group.replaceAll('_', '-'),
