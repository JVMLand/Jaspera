...
export function instructionUsage(op: string) {
  ...
  if ((m = op.match(/^([ilfda])(load|store)(?:_([0-3]))?$/))) {
    const [, t, action, n] = m,
      slot = n ?? '1',
      instruction = n === undefined ? `${op} ${slot}` : op;
    lines =
      action === 'load'
        ? [value(t), `${t}store ${slot}`, instruction, discard(t)]
        : [value(t), instruction, `${t}load ${slot}`, discard(t)];
        ...
        : /^ldc/.test(op)
          ...
          : op === 'bipush'
            ...
            : op === 'sipush'
              ? 'sipush 1000'
              : op;
    const t = op === 'ldc2_w' ? 'l' : /^[ld]const/.test(op) ? op[0] : 'a';
    lines = [ins, discard(t)];
  } else if ((m = op.match(/^([ilfd])(add|sub|mul|div|rem|and|or|xor|shl|shr|ushr)$/))) {
    const t = m[1];
    lines = [value(t, 7), value(/sh/.test(m[2]) ? 'i' : t, 2), op, discard(t)];
    ...
    const t = m[1],
      array = t === 'a' ? 'anewarray Ljava/lang/String;' : `newarray ${descriptor[t]}`;
    lines = [
      'iconst_2',
      array,
      'astore_0',
      'aload_0',
      'iconst_0',
      value(t === 'b' || t === 'c' || t === 's' ? 'i' : t),
      `${t}astore`,
    ];
    if (m[2] === 'load') lines.push('aload_0', 'iconst_0', op, discard(t));
    else lines.push('aload_0', 'arraylength', 'pop');
  } else if (/^(get|put)(field|static)$/.test(op)) {
    members =
      '  public value:I\n  public static count:I\n  public <init>()V {\n    aload_0\n    invokespecial java/lang/Object-><init>()V\n    return\n  }\n';
    const instance = op.endsWith('field'),
      put = op.startsWith('put');
    lines = [
      ...(instance
        ? ['new InstructionExample', 'dup', 'invokespecial InstructionExample-><init>()V']
        : []),
      ...(put ? ['bipush 7'] : []),
      `${op} InstructionExample->${instance ? 'value' : 'count'}:I`,
      ...(!put ? ['pop'] : []),
    ];
    ...
    lines = ['bipush -7', 'invokestatic java/lang/Math->abs(I)I', 'pop'];
    ...
    lines = [
      'bipush 7',
      'invokedynamic makeConcat(I)Ljava/lang/String; MethodHandle|invokestatic|java/lang/invoke/StringConcatFactory->makeConcat(Ljava/lang/invoke/MethodHandles$Lookup;Ljava/lang/String;Ljava/lang/invoke/MethodType;)Ljava/lang/invoke/CallSite;',
      'pop',
    ];
  else if (/^[ilfda]?return$/.test(op)) {
    result = op === 'return' ? 'V' : descriptor[op[0]];
    lines = op === 'return' ? ['iconst_1', 'istore_0', 'return'] : [value(op[0]), op];
  } else if (op === 'athrow')
    lines = [
    ...
    const ref = /acmp|null/.test(op),
      pair = /cmp/.test(op);
    lines = [
      ref ? 'aconst_null' : 'iconst_1',
      ...(pair ? [ref ? 'aconst_null' : 'iconst_2'] : []),
      `${op} Match`,
      'iconst_0',
      'ireturn',
      'Match:',
    ...
    lines = [
      'iconst_1',
      op === 'tableswitch'
        ? 'tableswitch 0 { Zero, One } default Other'
    ...
    lines = ['iconst_3', 'anewarray Ljava/lang/String;', 'arraylength', 'pop'];
  else if (op === 'multianewarray')
    lines = ['iconst_2', 'iconst_3', 'multianewarray [[I 2', 'arraylength', 'pop'];
    ...
    lines = ['aload_0', 'monitorenter', 'iconst_1', 'istore_1', 'aload_0', 'monitorexit'];
    ...
    const count = operands[op];
    if (!count) throw new Error(`Missing usage: ${op}`);
    const remaining =
      op === 'pop'
        ? 0
        : op === 'pop2'
          ...
          : op === 'swap'
            ? count
            : count + (op.startsWith('dup2') ? 2 : 1);
    lines = [
      ...Array.from({ length: count }, (_, i) => `iconst_${i + 1}`),
      op,
      ...Array.from({ length: remaining }, () => 'pop'),
    ];
  }
