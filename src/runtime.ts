...
export class Runtime {
 ...
 run(compilation:Compilation,stdin:string,debug?:DebugOptions){return this.request({type:'run',compilation,stdin,debug},debug?0:30_000);}
 debugCommand(command:DebugCommand){return this.rpc.call(api=>api.debugCommand(command));}
 debugBreakpoints(points:DebugBreakpoint[]){return this.rpc.call(api=>api.debugBreakpoints(points));}
