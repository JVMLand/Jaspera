...
interface MethodView {name:string;step:number;phase:string;group:SVGGElement;graph?:MethodGraph;layout?:Layout;top?:number;paintKey?:string}
export function installInstructionGraph(host:HTMLElement,compile:(doc:GraphDocument,onProgress?:(progress:AnalysisProgress)=>void)=>Promise<Compilation>,navigate:(doc:GraphDocument,line:number,column:number)=>void){
 ...
 const lineNodes=new Map<number,{id:string;column:number}[]>();let selectedElement:SVGGElement|null=null;
 function highlight(){const at=lineNodes.get(doc?.line??0)?.filter(n=>n.column<=(doc?.column??0)).at(-1);selectedElement?.classList.remove('selected');selectedElement=at?scene.querySelector<SVGGElement>(`.graph-node[data-id="${CSS.escape(at.id)}"]`):null;selectedElement?.classList.add('selected');}
 ...
 function reflow(){let top=0;width=420;for(const method of methods){method.top=top;method.group.setAttribute('transform',`translate(0,${top})`);const w=method.layout?.width??Math.max(420,Math.min(1000,method.name.length*7+32));width=Math.max(width,w);top+=(method.layout?.height??108)+24;}height=Math.max(0,top-24);if(autoFit)fitWidth();schedulePaint();summary();}
 ...
 function render(method:MethodView,placed:Layout){method.layout=placed;method.paintKey=undefined;method.step=3;reflow();}
 function paintVisible(){
  if(disposed)return;
  const margin=160/scale,left=(-x)/scale-margin,right=(svg.clientWidth-x)/scale+margin,top=(-y)/scale-margin,bottom=(svg.clientHeight-y)/scale+margin;
  for(const method of methods){
   const placed=method.layout;if(!placed)continue;
 ...
 function refresh(next?:GraphDocument){
  ...
  clearTimeout(timer);const id=++ticket;++layoutTicket;worker.stop();layoutQueue=Promise.resolve();scene.replaceChildren();lineNodes.clear();selectedElement=null;methods=[];failure='';autoFit=true;
 ...
 return {update,dispose(){visibility.dispose();disposed=true;ticket++;layoutTicket++;clearTimeout(timer);cancelAnimationFrame(paintFrame);resize.disconnect();context.dispose();worker.dispose();host.replaceChildren();}};
}
