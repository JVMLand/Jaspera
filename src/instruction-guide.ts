...
export function guide(op: string) {
  ...
  if (m) {
    ...
    example = m[3] ? op : op + ' 1';
    if (m[2] === 'load') {
      summary = `ローカル変数 #${slot} の ${type} 値を，計算用のスタックの一番上へコピーします。`;
      add([], [value]);
    } else {
      summary = `スタックの一番上の ${type} 値を取り出し，ローカル変数 #${slot} に保存します。`;
      add([value], [], undefined, {
        before: [`#${slot}: 以前の値`],
        after: [`#${slot}: ${value}`],
      });
    ...
    summary =
      'ローカル変数の int 値に指定した数を足します。スタックを使わず，変数を直接更新します。';
    example = 'iinc 1 1';
    add([], [], '例では #1 を 1 増やします。', { before: ['#1: 3 : int'], after: ['#1: 4 : int'] });
        ...
        : op.startsWith('ldc')
          ...
          : op.endsWith('m1')
            ...
            : op.includes('const_')
              ? op.split('_')[1] + ' : ' + type
              : '整数 : int';
    summary = '指定した定数をスタックの一番上に積み，後の命令が使えるようにします。';
    ...
    if (['shl', 'shr', 'ushr'].includes(operation)) {
      summary = `${type} の値を${operation === 'shl' ? '左' : '右'}へずらします。TOP の int 値がずらすビット数，その下の値が対象です。`;
      add([`ずらす値 : ${type}`, 'ビット数 : int'], [`ずらした値 : ${type}`]);
    } else {
      const ordered = ['sub', 'div', 'rem'].includes(operation);
      summary = `スタックから2つの ${type} 値を取り出して${descriptions[operation]}を行い，結果を1つ積みます。`;
      if (ordered)
        summary +=
          operation === 'sub'
            ? 'TOP の値を，その下の値から引きます。'
      ...
      const bitNote: Record<string, string> = {
        and: '両方のビットが 1 の位置だけ，結果を 1 にします。',
        or: '少なくとも片方のビットが 1 なら，結果を 1 にします。',
        xor: '2つのビットが異なる位置だけ，結果を 1 にします。',
      };
      add(
    ...
    summary = `スタックの値を ${type} から ${types[op[2]]} に変換します。`;
    add(
      [`値 : ${type}`],
      [`変換した値 : ${'bcs'.includes(op[2]) ? 'int' : types[op[2]]}`],
      'bcs'.includes(op[2])
        ? '指定した型の幅に狭めたあと，スタック上では int として扱います。byte / short は符号拡張，char はゼロ拡張されます。'
        : undefined,
    );
    ...
    summary = store
      ? '配列・添字・値をスタックから取り出し，配列の指定要素へ保存します。'
      : '配列と添字をスタックから取り出し，配列の指定要素を読み出します。';
    add(
      ['配列への参照', '添字 : int', ...(store ? [`値 : ${valueType}`] : [])],
      store ? [] : [`要素 : ${valueType}`],
      'bcs'.includes(op[0])
        ? 'byte・boolean・char・short の配列要素も，スタック上では int です。'
        : undefined,
    );
  } else if (/^(get|put)(field|static)$/.test(op)) {
    const put = op.startsWith('put'),
      instance = op.endsWith('field');
    summary = `${instance ? 'オブジェクトの' : 'クラスに属する static な'}フィールドを${put ? '書き換えます' : '読み出します'}。${instance ? '対象オブジェクトの参照もスタックに必要です。' : '対象オブジェクトの参照は不要です。'}`;
    example =
      op +
      ' ' +
      (instance
    ...
    add(
      [...(instance ? ['対象オブジェクト'] : []), ...(put ? ['フィールドの型に合う値'] : [])],
      put ? [] : ['フィールドの値'],
      '型は : の後ろの descriptor で決まります。',
    );
  } else if (op.startsWith('invoke')) {
    const instance = !['invokestatic', 'invokedynamic'].includes(op);
    summary = `${instance ? '対象オブジェクトと引数' : '引数'}をスタックから取り出し，メソッドを呼び出します。戻り値があれば，戻ってきたときにスタックへ積まれます。`;
    example =
      op === 'invokevirtual'
        ...
        : op === 'invokestatic'
          ...
          : op;
    add(
      [...(instance ? ['対象オブジェクト'] : []), '引数1', '… 引数N'],
      ['戻り値'],
      '戻り型が V (void) なら戻り値は積まれません。',
      undefined,
      '値を返すメソッド',
    );
    add(
      [...(instance ? ['対象オブジェクト'] : []), '引数1', '… 引数N'],
      [],
      'descriptor の括弧内が引数，括弧の後ろが戻り型です。',
      undefined,
      'void を返すメソッド',
    );
    ...
    summary =
      op === 'return'
        ? '現在のメソッドを終了し，値を返さず呼び出し元へ戻ります。'
        : `${type} の値を取り出して呼び出し元へ返し，現在のメソッドを終了します。`;
    add(
      op === 'return' ? [] : [`戻り値 : ${type}`],
      ['メソッド終了'],
      '現在のスタックとローカル変数配列は破棄されます。図の右側は同じフレームの続きではありません。',
    );
  } else if (op === 'athrow')
    add(
      ['例外オブジェクト'],
      ['ハンドラー: 例外オブジェクト'],
      '通常の次の命令には進みません。対応するハンドラーのスタックは例外参照1つで開始します。見つからなければ呼び出し元へ伝播します。',
    );
  else if (/^if/.test(op)) {
    summary =
      '条件を調べ，成立すれば指定したラベルへ進みます。不成立なら次の命令へ進みます。比較に使った値はどちらの場合も取り出されます。';
    example = op + ' Label';
    add(/cmp/.test(op) ? ['比較する値1', '比較する値2'] : ['比較する値'], []);
    ...
    add(
      ...
      op === 'lcmp'
        ? '比較結果を積みます。分岐は行いません。'
        : `どちらかが NaN なら ${op.endsWith('l') ? '−1' : '1'} を積みます。`,
    );
  else if (/switch$/.test(op))
    add(['分岐キー : int'], [], '一致する分岐先，なければ default へ進みます。');
  else if (/^goto/.test(op)) {
    example = op + ' Label';
    add([], [], '値を動かさず，指定したラベルへ進みます。');
  } else if (/^jsr/.test(op))
    add(
      [],
      ['戻り先アドレス'],
      '古い finally 実装用です。現代の class ファイルでは使用制約があります。',
    );
  else if (op === 'ret')
    add(
      [],
      [],
      'ローカル変数 N にある戻り先アドレスへ進みます。通常のメソッド復帰には return 系を使います。',
    );
  else if (op === 'new') {
    ...
    add(
      [],
      ['未初期化のオブジェクト参照'],
      '生成後は invokespecial で <init> を呼び出して初期化します。',
    );
  } else if (/^(a?newarray|multianewarray)$/.test(op))
    add(op === 'multianewarray' ? ['各次元の長さ : int'] : ['長さ : int'], ['配列への参照']);
  else if (op === 'arraylength') add(['配列への参照'], ['長さ : int']);
  else if (op === 'checkcast')
    add(
      ['参照'],
      ['同じ参照'],
      '対象の型として扱えるかを検査します。新しいオブジェクトを作る命令ではありません。',
    );
  else if (op === 'instanceof')
    add(['参照'], ['適合する: 1 / しない: 0'], '結果の型は int です。null の結果は 0 です。');
  else if (op.startsWith('monitor'))
    add(
      ['対象オブジェクト'],
      [],
      op === 'monitorenter' ? '対象のモニターを取得します。' : '対象のモニターを解放します。',
    );
    ...
    add(
      [],
      ['値 : int'],
      'wide iload の例です。wide は後続命令のインデックス等を広く符号化します。前後の変化は対象命令と同じです。',
    );
  } else {
    ...
    pairs.forEach((m, i) =>
      add(
        ...
        m[3]
          ...
          .map((s) => s.trim())
          .filter((s) => s !== '...'),
        m[2].trim() || '値のカテゴリ制約は詳細解説を確認してください。',
        undefined,
        pairs.length > 1 ? '形式 ' + (i + 1) : '基本の形',
      ),
    );
  }
  const constraints: Record<string, string[]> = {
    pop: ['value はカテゴリ1です。'],
    pop2: ['value1・value2 はカテゴリ1です。', 'value はカテゴリ2です。'],
    dup: ['value はカテゴリ1です。'],
    swap: ['value1・value2 はカテゴリ1です。'],
    dup_x1: ['すべての値はカテゴリ1です。'],
    dup_x2: [
      'すべての値はカテゴリ1です。',
      'TOP の value1 はカテゴリ1，その下の value2 はカテゴリ2です。',
    ],
    dup2: ['value1・value2 はカテゴリ1です。', 'value はカテゴリ2です。'],
    dup2_x1: [
      'すべての値はカテゴリ1です。',
      'TOP の value1 はカテゴリ2，その下の value2 はカテゴリ1です。',
    ],
    dup2_x2: [
      'すべての値はカテゴリ1です。',
      'TOP の value1 はカテゴリ2，その下の value2・value3 はカテゴリ1です。',
      'TOP の value1・value2 はカテゴリ1，その下の value3 はカテゴリ2です。',
      'value1・value2 はカテゴリ2です。',
    ],
  };
        ...
        : op === 'ifnonnull'
          ...
          : /cmp/.test(op)
            ? ['eq', 'ne'].includes(op.slice(-2))
              ? `2つの値が${relation[op.slice(-2)]}`
              : `TOP の下の値が TOP の値と比較して「${relation[op.slice(-2)]}」`
            : `TOP の int 値が 0 と比較して「${relation[op.slice(-2)]}」`;
    summary = `${condition}なら，指定したラベルへ進みます。成立しなければ次の命令へ進みます。比較に使った値は，どちらの場合もスタックから取り出されます。`;
  }
  const summaries: Record<string, string> = {
    invokevirtual:
      '対象オブジェクトの実際のクラスに従ってインスタンスメソッドを呼びます。オーバーライドが反映されます。',
    invokeinterface: 'インターフェースのメソッドを，そのオブジェクトが実装する処理で呼び出します。',
    invokespecial:
      'コンストラクターや親クラスのメソッドを，通常の仮想呼び出しと異なる規則で呼びます。',
    invokestatic:
      '指定した static メソッドを呼びます。スタックに積むのは引数だけで，対象オブジェクトは不要です。',
    invokedynamic:
      'bootstrap メソッドで結び付けた呼び出し先を実行します。引数と戻り値の型は descriptor で決まります。',
    ...
    checkcast:
      '参照を指定した型として扱えるか検査します。成功すれば同じ参照を残し，失敗すれば例外を投げます。',
  ...
  if (op === 'aconst_null')
    summary = 'どのオブジェクトも指していない参照 null をスタックへ積みます。';
  if ((m = op.match(/^([ilfd])const_(m1|[0-5])$/)))
    summary = `${type} の定数 ${m[2] === 'm1' ? '-1' : m[2]} をスタックへ積みます。`;
  if (/^[bs]ipush$/.test(op))
    summary = `命令に書いた ${op === 'bipush' ? '8' : '16'} ビットの符号付き整数を，int としてスタックへ積みます。`;
  if (/^ldc/.test(op))
    summary =
      op === 'ldc2_w'
        ? '定数プールから long / double の定数を読み出して積みます。'
        : '定数プールから文字列や int / float などの定数を読み出して積みます。' +
          (op === 'ldc_w' ? '定数プールの番号を広く指定できる形式です。' : '');
  if (/^[ilfd]neg$/.test(op))
    summary = `スタックの ${type} 値を取り出し，符号を反転して積み直します。`;
  if (/^[lfd]cmp/.test(op))
    summary = `2つの ${type} 値を比較し，大小関係を int の −1 / 0 / 1 として積みます。分岐そのものは行いません。`;
  if (/^goto/.test(op)) summary = 'スタックの値を変更せず，指定したラベルへ無条件に分岐します。';
  if (/^jsr/.test(op))
    summary = '次の命令の戻り先アドレスを積み，同じメソッド内のサブルーチンへ分岐します。';
  if (/^if_acmp/.test(op))
    summary = `2つの参照が${op.endsWith('eq') ? '同じオブジェクトを指す（または両方 null）' : '異なる'}なら指定したラベルへ分岐します。比較した参照は両方とも取り除きます。`;
  if (/^[ilfdabcs]a(load|store)$/.test(op))
    summary = `${type} 配列の指定した添字の要素を${op.endsWith('store') ? '書き換えます。スタックから配列・添字・値を取り出します。' : '読み出します。配列と添字を取り出し，要素の値を積みます。'}`;
