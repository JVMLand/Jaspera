...
export function guide(op:string){
 ...
 if(m){const slot=m[3]??'N',value=`値 : ${type}`;example=m[3]?op:op+' 1';if(m[2]==='load'){summary=`ローカル変数 #${slot} の ${type} 値を、計算用のスタックの一番上へコピーします。`;add([], [value],`ローカル変数 #${slot} の値はそのまま残ります。`);}else{summary=`スタックの一番上の ${type} 値を取り出し、ローカル変数 #${slot} に保存します。`;add([value],[],undefined,{before:[`#${slot}: 以前の値`],after:[`#${slot}: ${value}`]});}}
 ...
 else if(m=op.match(/^([ilfd])(add|sub|mul|div|rem|and|or|xor|shl|shr|ushr)$/)){const symbols:Record<string,string>={add:'+',sub:'−',mul:'×',div:'÷',rem:'の余り',and:'AND',or:'OR',xor:'XOR',shl:'<<',shr:'>>',ushr:'>>>'};summary=`スタックから2つの値を取り出して${doc.title}を行い、結果を1つ積みます。先に積んだ値が左側、TOP の値が右側です。`;add([`左の値 : ${type}`,`右の値 : ${/sh/.test(m[2])?'int':type}`],[`左 ${symbols[m[2]]} 右 : ${type}`]);}
 else if(/^[ilfd]neg$/.test(op))add([`値 : ${type}`],[`−値 : ${type}`]);
 else if(/^[ilfd]2/.test(op)){summary=`スタックの値を ${type} から ${types[op[2]]} に変換します。変換によって精度や値の範囲が変わる場合があります。`;add([`値 : ${type}`],[`変換した値 : ${'bcs'.includes(op[2])?'int':types[op[2]]}`],'bcs'.includes(op[2])?'指定した型の幅に狭めたあと、スタック上では int として扱います。byte / short は符号拡張、char はゼロ拡張されます。':undefined);}
 else if(/^[ilfdabcs]a(load|store)$/.test(op)){const store=op.endsWith('store'),valueType='bcs'.includes(op[0])?'int':type;summary=store?'配列・添字・値をスタックから取り出し、配列の指定要素へ保存します。':'配列と添字をスタックから取り出し、配列の指定要素を読み出します。';add(['配列への参照','添字 : int',...(store?[`値 : ${valueType}`]:[])],store?[]:[`要素 : ${valueType}`],'配列の中身はヒープ上にあります。ローカル変数配列とは別物です。'+('bcs'.includes(op[0])?' byte・boolean・char・short の配列要素も、スタック上では int です。':''));}
 else if(/^(get|put)(field|static)$/.test(op)){const put=op.startsWith('put'),instance=op.endsWith('field');summary=`${instance?'オブジェクトの':'クラスに属する static な'}フィールドを${put?'書き換えます':'読み出します'}。${instance?'対象オブジェクトの参照もスタックに必要です。':'対象オブジェクトの参照は不要です。'}`;example=op+' '+(instance?'Example->value:I':'java/lang/System->out:Ljava/io/PrintStream;');add([...(instance?['対象オブジェクト']:[]),...(put?['フィールドの型に合う値']:[])],put?[]:['フィールドの値'],'型は : の後ろの descriptor で決まります。フィールドはローカル変数とは別の保存場所です。');}
 else if(op.startsWith('invoke')){const instance=!['invokestatic','invokedynamic'].includes(op);summary=`${instance?'対象オブジェクトと引数':'引数'}をスタックから取り出し、メソッドを呼び出します。戻り値があれば、戻ってきたときにスタックへ積まれます。`;example=op==='invokevirtual'?'invokevirtual java/io/PrintStream->println(Ljava/lang/String;)V':op==='invokestatic'?'invokestatic java/lang/Math->abs(I)I':op;add([...(instance?['対象オブジェクト']:[]),'引数1','… 引数N'],['戻り値'],'戻り型が V (void) なら戻り値は積まれません。呼び出し先には別のローカル変数配列が作られます。',undefined,'値を返すメソッド');add([...(instance?['対象オブジェクト']:[]),'引数1','… 引数N'],[],'descriptor の括弧内が引数、括弧の後ろが戻り型です。',undefined,'void を返すメソッド');}
 ...
 if(/^if/.test(op)){
  ...
  const condition=op==='ifnull'?'参照が null':op==='ifnonnull'?'参照が null 以外':/cmp/.test(op)?`先に積んだ値が TOP の値と比較して「${relation[op.slice(-2)]}」`:`TOP の int 値が 0 と比較して「${relation[op.slice(-2)]}」`;
 ...
 return {op,category:category(op),title:doc.title,summary,example,forms,markdown};
}
