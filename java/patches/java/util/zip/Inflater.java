...
public class Inflater implements AutoCloseable {
 ...
 public Inflater(boolean nowrap){this.nowrap=nowrap;open();}
 public Inflater(){this(false);}
 private void open(){try{engine=new com.jcraft.jzlib.Inflater(nowrap);engine.next_in=new byte[0];}catch(com.jcraft.jzlib.GZIPException e){throw new IllegalArgumentException(e);}}
 private void ensureOpen(){if(engine==null)throw new NullPointerException("Inflater has been closed");}
 ...
 public synchronized void setDictionary(byte[] b,int off,int len){ensureOpen();Objects.checkFromIndexSize(off,len,b.length);if(engine.setDictionary(Arrays.copyOfRange(b,off,off+len),len)!=0)throw new IllegalArgumentException("Invalid dictionary");dictionary=false;}
 ...
 public synchronized int getRemaining(){ensureOpen();return engine.avail_in;}
 public boolean needsInput(){return getRemaining()==0;}
 public synchronized boolean needsDictionary(){return dictionary;}
 public synchronized boolean finished(){return finished;}
 public int inflate(byte[] b) throws DataFormatException{return inflate(b,0,b.length);}
 public synchronized int inflate(byte[] b,int off,int len) throws DataFormatException{
  ensureOpen();Objects.checkFromIndexSize(off,len,b.length);if(len==0||finished)return 0;
  engine.next_out=b;engine.next_out_index=off;engine.avail_out=len;int before=engine.avail_in;
  int result=engine.inflate(0);
  if(inputBuffer!=null)inputBuffer.position(inputBuffer.position()+before-engine.avail_in);
  if(result==1)finished=true;else if(result==2)dictionary=true;
 ...
 public int inflate(ByteBuffer b) throws DataFormatException{if(b.isReadOnly())throw new java.nio.ReadOnlyBufferException();byte[] bytes=new byte[b.remaining()];int n=inflate(bytes);b.put(bytes,0,n);return n;}
 ...
 public synchronized void reset(){ensureOpen();engine.end();open();finished=dictionary=pendingOutput=false;inputBuffer=null;}
 public synchronized void end(){if(engine!=null){engine.end();engine=null;}inputBuffer=null;}
 public void close(){end();}
 boolean hasPendingOutput(){return pendingOutput;}
}
