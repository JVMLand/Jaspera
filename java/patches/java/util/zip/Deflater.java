...
public class Deflater implements AutoCloseable {
    ...
    private void open() {
        engine = new com.jcraft.jzlib.Deflater();
        engine.next_in = new byte[0];
        if (engine.init(level, nowrap) != 0) throw new IllegalArgumentException(
            "Invalid compression level"
        );
    }

    private void ensureOpen() {
        if (engine == null) throw new NullPointerException("Deflater has been closed");
    }
    ...
    public synchronized void setStrategy(int s) {
        if (s < 0 || s > 2) throw new IllegalArgumentException("Invalid strategy");
        strategy = s;
        parametersChanged = true;
    }

    public synchronized void setLevel(int l) {
        if (l < -1 || l > 9) throw new IllegalArgumentException("Invalid level");
        level = l;
        parametersChanged = true;
    }
    ...
    public synchronized int deflate(byte[] b, int off, int len, int flush) {
        ...
        Objects.checkFromIndexSize(off, len, b.length);
        if (
            flush != NO_FLUSH && flush != SYNC_FLUSH && flush != FULL_FLUSH
        ) throw new IllegalArgumentException("Invalid flush mode");
        ...
        if (parametersChanged) {
            result = engine.params(level, strategy);
    ...
    public int deflate(ByteBuffer b) {
        return deflate(b, NO_FLUSH);
    }

    public int deflate(ByteBuffer b, int flush) {
        if (b.isReadOnly()) throw new java.nio.ReadOnlyBufferException();
        byte[] bytes = new byte[b.remaining()];
        int n = deflate(bytes, 0, bytes.length, flush);
        b.put(bytes, 0, n);
        return n;
    }
