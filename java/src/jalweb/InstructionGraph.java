...
final class InstructionGraph {
    ...
    static String compute(String owner,MethodNode method,Frame<BasicValue>[] frames) throws AnalyzerException {
        ...
        for(TryCatchBlockNode handler:method.tryCatchBlocks)leaders.add(real(handler.handler));
        int block=-1;
        for(int i=0;i<method.instructions.size();i++){
            ...
            nodes.add("{\"id\":"+Bridge.quote(id)+",\"text\":"+Bridge.quote(text)+",\"opcode\":"+Bridge.quote(source.start.getText())+",\"block\":"+Bridge.quote("B"+block)+",\"line\":"+source.start.getLine()+",\"column\":"+(source.start.getCharPositionInLine()+1)+",\"consumed\":"+consumed+",\"produced\":"+produced+",\"unreachable\":"+(before==null)+"}");
            int opcode=insn.getOpcode();
            ...
            for(TryCatchBlockNode handler:method.tryCatchBlocks)if(i>=method.instructions.indexOf(handler.start)&&i<method.instructions.indexOf(handler.end))edge(edges,id,ids.get(real(handler.handler)),"exception",handler.type==null?"finally":handler.type.substring(handler.type.lastIndexOf('/')+1));
        }
