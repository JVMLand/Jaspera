...
final class InstructionGraph {
    ...
    static final class Inputs extends Frame<SourceValue> {
        ...
        Inputs(Frame<SourceValue> frame) {
            super(frame);
        }
    ...
    static String compute(String owner, MethodNode method, Frame<BasicValue>[] frames)
        ...
        for (AbstractInsnNode insn : method.instructions) {
            ...
            ) leaders.add(real(insn.getNext()));
            if (insn instanceof JumpInsnNode jump) {
                leaders.add(real(jump.label));
                leaders.add(real(insn.getNext()));
            }
            if (insn instanceof TableSwitchInsnNode sw) {
                leaders.add(real(sw.dflt));
                for (LabelNode label : sw.labels) leaders.add(real(label));
            }
            if (insn instanceof LookupSwitchInsnNode sw) {
                leaders.add(real(sw.dflt));
                for (LabelNode label : sw.labels) leaders.add(real(label));
            }
        }
        for (TryCatchBlockNode handler : method.tryCatchBlocks) {
            leaders.add(real(handler.start));
            leaders.add(real(handler.end));
            leaders.add(real(handler.handler));
        }
        Map<AbstractInsnNode, String> blocks = new IdentityHashMap<>();
        int block = -1;
        for (AbstractInsnNode insn : method.instructions)
            if (ids.containsKey(insn)) {
        ...
        for (int i = 0; i < method.instructions.size(); i++) {
            ...
            String text = source.start
                .getInputStream()
                .getText(Interval.of(source.start.getStartIndex(), source.stop.getStopIndex()))
                .replaceAll("\\s+", " ");
            ...
            if (before != null) {
                ...
                produced = after.getStackSize() - after.untouched;
                Inputs input = new Inputs(sources[i]);
                input.execute(insn, interpreter);
                for (SourceValue value : input.consumed)
                    dependencies(edges, ids, value, id, "stack");
                int local =
                    insn instanceof IincInsnNode inc
                        ? inc.var
                        : insn instanceof VarInsnNode variable &&
                            (insn.getOpcode() <= Opcodes.ALOAD || insn.getOpcode() == Opcodes.RET)
                          ? variable.var
                          : -1;
            ...
            nodes.add(
                "{\"id\":" +
                    ...
                    Bridge.quote(text) +
                    ",\"opcode\":" +
                    Bridge.quote(source.start.getText()) +
                    ",\"block\":" +
                    Bridge.quote(blocks.get(insn)) +
                    ",\"line\":" +
            ...
            if (insn instanceof JumpInsnNode jump) {
                edge(
                    edges,
                    id,
                    ids.get(real(jump.label)),
                    "control",
                    opcode == Opcodes.GOTO ? "jump" : opcode == Opcodes.JSR ? "call" : "true"
                );
                if (opcode != Opcodes.GOTO && opcode != Opcodes.JSR) edge(
                    edges,
                    id,
                    ids.get(real(insn.getNext())),
                    "control",
                    "false"
                );
            } else if (insn instanceof TableSwitchInsnNode sw) {
                edge(edges, id, ids.get(real(sw.dflt)), "control", "default");
                for (int n = 0; n < sw.labels.size(); n++) edge(
                    edges,
                    id,
                    ids.get(real(sw.labels.get(n))),
                    "control",
                    Integer.toString(sw.min + n)
                );
                ...
                for (int n = 0; n < sw.labels.size(); n++) edge(
                    ...
                    ids.get(real(sw.labels.get(n))),
                    "control",
                    sw.keys.get(n).toString()
                );
            } else if (opcode == Opcodes.RET) {
                for (int next : control.getOrDefault(i, Set.of()))
                    edge(
                        edges,
                        id,
                        ids.get(real(method.instructions.get(next))),
                        "control",
                        "return"
                    );
            } else if (
                ...
                opcode != Opcodes.ATHROW &&
                opcode != Opcodes.RET
            ) edge(edges, id, ids.get(real(insn.getNext())), "control", "");
        }
        ...
        for (TryCatchBlockNode handler : method.tryCatchBlocks) {
            ...
            for (
                AbstractInsnNode insn = handler.start;
                insn != null && insn != handler.end;
                insn = insn.getNext()
            ) {
                String group = blocks.get(insn);
                if (group != null) protectedBlocks.add(group);
            }
            String target = blocks.get(real(handler.handler));
            if (target != null) for (String group : protectedBlocks)
                exceptions
                    .computeIfAbsent(new ExceptionRoute(group, target), key ->
                        new LinkedHashSet<>()
                    )
                    .add(
                        handler.type == null
                            ? "finally"
                            : handler.type.substring(handler.type.lastIndexOf('/') + 1)
                    );
        }
        for (var route : exceptions.entrySet())
            edge(
                edges,
                route.getKey().from(),
                route.getKey().to(),
    ...
    static void dependencies(
        Set<String> edges,
        Map<AbstractInsnNode, String> ids,
        SourceValue value,
        String target,
        String kind
    ) {
        if (value != null) for (AbstractInsnNode producer : value.insns)
            edge(edges, ids.get(producer), target, kind, "");
    }

    static void edge(Set<String> edges, String from, String to, String kind, String label) {
        if (from != null && to != null) edges.add(
            "{\"from\":" +
                Bridge.quote(from) +
                ",\"to\":" +
                Bridge.quote(to) +
