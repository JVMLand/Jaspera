...
final class StackDiagnostics {
    ...
    static ParserRuleContext find(ParserRuleContext node, String rule) {
        ...
        for (int i = 0; i < node.getChildCount(); i++) if (
            node.getChild(i) instanceof ParserRuleContext child
        ) {
            ParserRuleContext found = find(child, rule);
    ...
    private static Range span(ParserRuleContext node, Range fallback) {
        return node == null
            ...
            : new Range(
                  node.start.getLine(),
                  node.start.getCharPositionInLine(),
                  node.stop.getStopIndex() - node.start.getStartIndex() + 1
              );
    }

    private static String typeName(StackElement element) {
        if (element instanceof StackElementCapsule capsule) return capsule.getElement() != null
            ? typeName(capsule.getElement())
            : capsule.isAcceptsCategoryTwoTop()
              ? "スタックの値"
              : "カテゴリ1の値";
        if (element instanceof ObjectElement object) return Type.getType(
        ...
        if (error.consumedSlots >= 0) {
            String descriptor =
                instruction instanceof MethodInsnNode method
                    ? method.desc
                    : instruction instanceof InvokeDynamicInsnNode dynamic
                      ? dynamic.desc
                      : null;
            if (descriptor != null) {
                ...
                for (int i = arguments.length - 1; i >= 0; i--) {
                    if (error.consumedSlots < slots + arguments[i].getSize()) {
                        argument = i;
                        break;
                    }
                    slots += arguments[i].getSize();
                }
                if (argument >= 0) {
                    ParserRuleContext desc = find(source, "methodDescriptor");
                    int offset = 1;
                    for (int i = 0; i < argument; i++) offset += arguments[i]
                        .getDescriptor()
                        .length();
                    if (desc != null) range = new Range(
                        desc.start.getLine(),
                        desc.start.getCharPositionInLine() + offset,
                        arguments[argument].getDescriptor().length()
                    );
                    expected = arguments[argument].getClassName();
                    subject = "第" + (argument + 1) + "引数";
                } else if (
                    instruction instanceof MethodInsnNode method &&
                    method.getOpcode() != org.objectweb.asm.Opcodes.INVOKESTATIC
                ) {
                    range = span(
                        find(source, "arrayTypeDescriptor") != null
                            ? find(source, "arrayTypeDescriptor")
                            : find(source, "fullQualifiedClassName"),
                        range
                    );
                    expected = Type.getObjectType(method.owner).getClassName();
                    subject = "呼び出し先のオブジェクト";
                }
            } else if (instruction instanceof FieldInsnNode field) {
                boolean value =
                    (field.getOpcode() == org.objectweb.asm.Opcodes.PUTSTATIC ||
                        field.getOpcode() == org.objectweb.asm.Opcodes.PUTFIELD) &&
                    error.consumedSlots < Type.getType(field.desc).getSize();
                range = span(
                    find(source, value ? "typeDescriptor" : "fullQualifiedClassName"),
                    range
                );
                expected = value
                    ? Type.getType(field.desc).getClassName()
                    : Type.getObjectType(field.owner).getClassName();
                subject = value ? "フィールドに代入する値" : "フィールドの参照先オブジェクト";
            }
        }
        String message =
            expected == null
