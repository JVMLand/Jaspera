...
final class StackFrames {
    static final class Verifier extends BasicVerifier {
        ...
        @Override protected boolean isSubTypeOf(BasicValue value,BasicValue expected){
            return value.isReference()&&expected.isReference()||super.isSubTypeOf(value,expected);
        }
    ...
    static String value(BasicValue value,SourceValue origin){
        String type=value(value);if(origin==null||origin.insns.size()!=1)return type;
        AbstractInsnNode producer=origin.insns.iterator().next();int opcode=producer.getOpcode();String constant=null;
        if(opcode==Opcodes.ACONST_NULL)return "null";
        if(opcode>=Opcodes.ICONST_M1&&opcode<=Opcodes.ICONST_5)constant=Integer.toString(opcode-Opcodes.ICONST_0);
        else if(opcode>=Opcodes.LCONST_0&&opcode<=Opcodes.LCONST_1)constant=Integer.toString(opcode-Opcodes.LCONST_0);
        else if(opcode>=Opcodes.FCONST_0&&opcode<=Opcodes.FCONST_2)constant=Integer.toString(opcode-Opcodes.FCONST_0);
        else if(opcode>=Opcodes.DCONST_0&&opcode<=Opcodes.DCONST_1)constant=Integer.toString(opcode-Opcodes.DCONST_0);
        else if(producer instanceof IntInsnNode operand&&(opcode==Opcodes.BIPUSH||opcode==Opcodes.SIPUSH))constant=Integer.toString(operand.operand);
        else if(producer instanceof LdcInsnNode ldc&&(ldc.cst instanceof String||ldc.cst instanceof Number))constant=ldc.cst instanceof String?Bridge.quote(ldc.cst.toString()):ldc.cst.toString();
    ...
    static String stack(Frame<BasicValue> frame,Frame<SourceValue> origins){
        ...
        for(int i=0;i<frame.getStackSize();i++)values.add(Bridge.quote(value(frame.getStack(i),origins==null?null:origins.getStack(i))));
        return "["+String.join(",",values)+"]";
    }
    static String locals(Frame<BasicValue> frame,Frame<SourceValue> origins){
        List<String> values=new ArrayList<>();
        int last=frame.getLocals()-1;while(last>=0&&frame.getLocal(last)==BasicValue.UNINITIALIZED_VALUE)last--;
        if(last>=0&&frame.getLocal(last).getSize()==2&&last+1<frame.getLocals())last++;
        for(int i=0;i<=last;i++) {
            BasicValue value=frame.getLocal(i);
            values.add(Bridge.quote(i>0&&frame.getLocal(i-1)!=null&&frame.getLocal(i-1).getSize()==2?"継続スロット":value(value,origins==null?null:origins.getLocal(i))));
        }
        return "["+String.join(",",values)+"]";
    }
