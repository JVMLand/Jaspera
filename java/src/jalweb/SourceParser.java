...
final class SourceParser {
    static JALParser.RootContext parse(CommonTokenStream tokens, BaseErrorListener errors,
                                       boolean validTokens, BiConsumer<Integer,Integer> progress) {
        int total=Math.max(1,tokens.size()-1);
        ParseTreeListener listener=new ParseTreeListener() {
            int reported; long last;
            public void visitTerminal(TerminalNode node) {}
            public void visitErrorNode(ErrorNode node) {}
            public void enterEveryRule(ParserRuleContext context) {}
            public void exitEveryRule(ParserRuleContext context) {
                if (!(context instanceof JALParser.MethodDefinitionContext) || context.stop==null) return;
                int completed=Math.min(total,context.stop.getTokenIndex()+1);
                long now=System.nanoTime();
                if(completed>reported && now-last>=100_000_000L){
                    reported=completed;last=now;progress.accept(completed,total);
                }
            }
        };
        if(progress!=null)progress.accept(0,total);
        if(validTokens){
            JALParser quick=new JALParser(tokens);quick.removeErrorListeners();
            ...
            try {
                ...
                return root;
            } catch(ParseCancellationException ignored){tokens.seek(0);}
        }
        JALParser parser=new JALParser(tokens);parser.removeErrorListeners();parser.addErrorListener(errors);
