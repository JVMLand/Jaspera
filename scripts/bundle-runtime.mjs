...
export async function bundleRuntime(){
 ...
 source=source.replace('const malloced = module._malloc(2 * str.length);','const encoded = new TextEncoder().encode(str); const malloced = module._malloc(encoded.length + 1);')
 ...
 .replace('const result = new TextEncoder().encodeInto(str, arr);','arr.set(encoded); const result = { written: encoded.length };');
 ...
 await build({stdin:{contents:source,resolveDir:'.cache/b-jvm/js',loader:'ts'},bundle:true,format:'esm',platform:'browser',outfile:'public/runtime/bovine.js',target:'es2022',logLevel:'warning'});
