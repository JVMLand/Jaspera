...
test('large graphs mount only the viewport and keep all methods accessible',{timeout:60000},async t=>{
 const base='http://127.0.0.1:5232',server=spawn(process.execPath,['node_modules/vite/bin/vite.js','--host','127.0.0.1','--port','5232','--strictPort'],{stdio:'pipe',windowsHide:true});t.after(()=>server.kill());for(let i=0;i<100;i++){try{if((await fetch(base)).ok)break;}catch{}await new Promise(r=>setTimeout(r,100));}
 const browser=await chromium.launch({channel:'msedge',headless:true});t.after(()=>browser.close());const page=await browser.newPage({viewport:{width:1000,height:800}});await page.goto(base+'/tests/harness.html');
 ...
 await page.evaluate(()=>window.panel.dispose());assert.equal(await page.locator('.graph-node').count(),0);
});
