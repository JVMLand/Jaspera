  ...
  async (t) => {
    ...
    await page.addInitScript(() => {
      window.showDirectoryPicker = async () =>
        (await navigator.storage.getDirectory()).getDirectoryHandle('layout-test', {
          create: true,
        });
    });
    ...
    const save = async () => {
      await menu('file', 'save-project');
      await page.waitForFunction(
        () => !(document.querySelector('#save-project')! as HTMLButtonElement).disabled,
      );
    ...
    const disk = () =>
      page.evaluate(async () =>
        JSON.parse(
          await (
            await (
              await (await navigator.storage.getDirectory()).getDirectoryHandle('layout-test')
    ...
    for (const name of ['src/Helper.jal', 'src/Closed.jal']) {
    ...
    await page.locator('.separator-0').focus();
    await page.keyboard.press('ArrowRight');
    await menu('view', 'swap-panes');
    ...
    assert.equal(before.editor.windows.length, 1);
    assert.deepEqual(before.editor.windows[0].tabs, ['source:src/Helper.jal']);
    assert.equal(before.editor.windows[0].active, 'panel:instructions');
    assert.equal(before.editor.windows[0].views['source:src/Helper.jal'].column, 7);
    assert.equal(
      before.editor.tabs.some((t) => t.key === 'source:src/Closed.jal'),
      false,
    );
    assert.equal(before.editor.dock.swapped, true);
    ...
    assert.deepEqual(
      after.editor.windows.map((w) => ({
        tabs: w.tabs,
        panels: w.panels,
        active: w.active,
        views: w.views,
      })),
      before.editor.windows.map((w) => ({
        tabs: w.tabs,
        panels: w.panels,
        active: w.active,
        views: w.views,
    ...
    // Metadata polling must not change the selected panel or restore an old layout.
    await page.locator('#console-tab').click();
    await page.evaluate(async () => {
      const root = await (await navigator.storage.getDirectory()).getDirectoryHandle('layout-test'),
        h = await root.getFileHandle('project.jalprj'),
        p = JSON.parse(await (await h.getFile()).text());
      p.name = 'Renamed';
      p.editor.dock.swapped = false;
      const w = await h.createWritable();
      await w.write(JSON.stringify(p));
      await w.close();
    });
    await page.waitForFunction(
      () => document.querySelector('#project-name')!.textContent === 'Renamed',
    );
    assert.equal(await page.locator('#console-tab').getAttribute('aria-selected'), 'true');
    assert.equal(await page.locator('.workspace.dock-swapped').count(), 1);
    await menu('file', 'project-properties-menu');
    assert.equal(await page.locator('#project-properties input').count(), 1);
    await page.keyboard.press('Escape');
    // Restore two windows even if the first attempt is blocked; retry on real gestures.
    await menu('file', 'new-project');
    await page.evaluate(async (saved) => {
      const root = await (await navigator.storage.getDirectory()).getDirectoryHandle('layout-test'),
        h = await root.getFileHandle('project.jalprj');
      saved.editor.windows.push({
        tabs: [],
        panels: ['problems'],
      ...
      const w = await h.createWritable();
      await w.write(JSON.stringify(saved));
      await w.close();
