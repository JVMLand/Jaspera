...
test('stack manipulation variants keep ordering and category restrictions',()=>{const f=guide('dup2_x2').forms;assert.equal(f.length,4);assert.deepEqual(f[3].before,['b','a']);assert.deepEqual(f[3].after,['a','b','a']);assert.match(f[3].note,/カテゴリ2/);assert.equal(guide('pop2').forms.length,2);assert.deepEqual(guide('swap').forms[0].after,['a','b']);});
