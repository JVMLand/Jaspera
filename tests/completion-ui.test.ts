...
test(
  ...
  async (t) => {
    ...
    await t.test('getstatic without a space automatically offers fields', async () => {
      await type('getstatic');
      await choose('java/lang/System->out:Ljava/io/PrintStream;');
      assert.match(await source(), /    getstatic java\/lang\/System->out:Ljava\/io\/PrintStream;/);
      assert.equal((await source()).includes('getstatic getstatic'), false);
    });
    await t.test('dot-qualified and short member names replace the whole operand', async () => {
      for (const typed of ['getstatic java.lang.System.out', 'getstatic System.out']) {
        ...
        await choose('java/lang/System->out:Ljava/io/PrintStream;');
        assert.match(await source(), /getstatic java\/lang\/System->out:Ljava\/io\/PrintStream;/);
        assert.equal((await source()).includes(typed), false);
      }
      for (const typed of ['invokevirtual print', 'invokevirtual System.out.println']) {
        await type(typed);
        await page.keyboard.press('Control+Space');
        await choose('java/io/PrintStream->println(Ljava/lang/String;)V');
        assert.match(
          await source(),
    ...
    await t.test('print and fully qualified aliases insert runnable JAL', async () => {
      for (const typed of ['print', 'System.out.println', 'java.lang.System.out.println']) {
        await type(typed);
        await page.keyboard.press('Control+Space');
        await choose('System.out.println');
        const result = await source();
        assert.match(result, /getstatic java\/lang\/System->out:/);
        assert.match(result, /ldc "Hello, World!"/);
        assert.match(result, /invokevirtual java\/io\/PrintStream->println/);
      }
      await page.locator('#run').click();
      await page.waitForFunction(
        () =>
          /実行が完了|実行に失敗|コンパイルエラー/.test(
      ...
      assert.equal(
        ...
        '実行が完了しました',
        (await page.locator('#problems').textContent())!,
      );
