...
function program(body, extra = '') {
  return `public class Test {public static main([Ljava/lang/String;)V {${body}\nreturn\n}\n${extra}\n}`;
}
test('JALWeb browser / real WebAssembly integration', { timeout: 240000 }, async (t) => {
  if (!process.env.JALWEB_TEST_URL) {
    const server = spawn(
      process.execPath,
      ['node_modules/vite/bin/vite.js', '--host', '127.0.0.1', '--port', '5178', '--strictPort'],
  ...
  await page.evaluate(async () => {
    ...
    globalThis.compiler = new Runtime();
    globalThis.Runtime = Runtime;
  });
  const compile = (source) =>
    page.evaluate((source) => globalThis.compiler.compile(source), source);
  const run = (compilation, stdin = '') =>
    page.evaluate(
      async ({ compilation, stdin }) => {
        const runtime = new globalThis.Runtime();
        let stdout = '',
          stderr = '';
        runtime.onOutput = (stream, text) => {
          if (stream === 'stdout') stdout += text;
          else stderr += text;
        };
        try {
          await runtime.run(compilation, stdin);
  ...
  await t.test(
    'Unicode source and stdout retain Japanese and supplementary characters',
    async () => {
      const result = await compile(hello.replaceAll('Hello, World!', 'こんにちは 🌏 café'));
      assert.ok(result.bytecode, JSON.stringify(result));
  ...
  for (const [name, source] of [
    ['syntax', program('ldc')],
    ['lexer', program('@')],
    ['stack underflow', program('pop')],
    ['stack type', program('iconst_1\nastore_1')],
    ['unknown label', program('goto missing')],
  ...
  await t.test('standard input is available to java.io', async () => {
    ...
    const result = await compile(source);
    assert.ok(result.bytecode, JSON.stringify(result.diagnostics));
    assert.equal((await run(result, 'A')).stdout, '65\n');
  });
  await t.test('OpenJDK standard library package coverage', async () => {
    ...
    const build = spawnSync(
      ...
      [
        ...
        '-d',
        'java/build/probes',
        'tests/StandardLibraryProbe.java',
      ],
      { encoding: 'utf8' },
    );
    assert.equal(build.status, 0, build.stderr);
    const bytecode = (await readFile('java/build/probes/StandardLibraryProbe.class')).toString(
      'base64',
    );
    const output = await run({ className: 'StandardLibraryProbe', bytecode, diagnostics: [] });
    console.log(output.stdout);
    assert.doesNotMatch(output.stdout, /FAIL/);
    assert.equal((output.stdout.match(/PASS/g) ?? []).length, 13);
  });
  await t.test(
    'ZIP / GZIP, small buffers, flush, dictionary, reset and ByteBuffer adapters',
    async () => {
      const build = spawnSync(
        'javac',
        ['--release', '21', '-d', 'java/build/probes', 'tests/CompressionProbe.java'],
        { encoding: 'utf8' },
      );
      assert.equal(build.status, 0, build.stderr);
      const bytecode = (await readFile('java/build/probes/CompressionProbe.class')).toString(
        'base64',
      );
  ...
  await t.test('stopping an infinite loop terminates its Worker and permits reuse', async () => {
    ...
    const result = await page.evaluate(async (compilation) => {
      const runtime = new globalThis.Runtime();
      const task = runtime.run(compilation, '').then(
        () => false,
        (error) => error.message.includes('停止'),
      );
  ...
  await t.test('Monaco UI: default source, Run and diagnostics', async () => {
    ...
    await page.keyboard.insertText(
      'public class Test {public static main([Ljava/lang/String;)V {pop\nreturn}}',
    );
    await page.waitForFunction(
      () => Number(document.querySelector('#problem-count')?.textContent) > 0,
    );
    ...
    await page.waitForFunction(() =>
      document.querySelector('.suggest-widget.visible')?.textContent?.includes('invokevirtual'),
    );
    ...
    await page.waitForFunction(() =>
      document
        ...
        ?.textContent?.includes('out:Ljava/io/PrintStream;'),
    );
    await page.keyboard.press('Escape');
    await edit(
      'public class Test {\n  public static main([Ljava/lang/String;)V {\n    getstatic java/lang/System->out:Ljava/io/PrintStream;\n    pop\n    return\n  }\n}',
    ...
    await page
      ...
      .filter({ hasText: /^getstatic$/ })
      .first()
      .hover();
    await page.waitForFunction(() =>
      document.querySelector('.stack-hover:not([hidden])')?.textContent?.includes('実行前'),
    );
