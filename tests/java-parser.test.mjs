...
test('Java fast parser preserves LL trees and diagnostics and reports source progress', async () => {
  const dir = '.cache/parser-test';
  await mkdir(dir, { recursive: true });
  await writeFile(
    dir + '/ParserProbe.java',
    `package jalweb;
 ...
 public class ParserProbe {
  ...
  const compile = spawnSync(
    'javac',
    ['-cp', 'public/runtime/jalweb-compiler.jar', '-d', dir, dir + '/ParserProbe.java'],
    { encoding: 'utf8' },
  );
  assert.equal(compile.status, 0, compile.stderr);
  const run = spawnSync(
    'java',
    ['-cp', dir + delimiter + 'public/runtime/jalweb-compiler.jar', 'jalweb.ParserProbe'],
