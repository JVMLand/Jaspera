...
test(
  ...
  { timeout: 150000 },
  async (t) => {
    const base = 'http://127.0.0.1:5231',
      server = spawn(
        process.execPath,
        ['node_modules/vite/bin/vite.js', '--host', '127.0.0.1', '--port', '5231', '--strictPort'],
    ...
    await show();
    await input.fill('Main.jal');
    await dialog.getByRole('option').filter({ hasText: 'Main.jal' }).first().waitFor();
    await input.press('Enter');
    await dialog.waitFor({ state: 'hidden' });
    await show();
    await input.fill('java.lang.System out');
    const result = dialog.getByRole('option').filter({ hasText: 'out:Ljava/io/PrintStream;' });
    await result.first().waitFor();
    await result.first().click();
    await dialog.waitFor({ state: 'hidden', timeout: 90000 });
    const text = await page.evaluate(async () => {
      const m = await import('/src/editor-platform.ts');
    ...
    assert.match(text.path, /java\/lang\/System/);
    assert.match(text.line, /out/);
    await show();
    await input.fill('PrintStream');
    ...
    await popup.keyboard.press('Shift');
    const pd = popup.getByRole('dialog', { name: 'どこでも検索' });
    await pd.waitFor();
    await pd.getByRole('combobox').fill('java.lang.System out');
    await pd.getByRole('option').filter({ hasText: 'out:Ljava/io/PrintStream;' }).first().click();
    await pd.waitFor({ state: 'hidden' });
    assert.ok(await popup.locator('#editor').innerText());
    await popup.close();
