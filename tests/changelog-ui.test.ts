...
import { launchBrowser, newAppContext } from './helpers/browser.ts';
const currentVersion = JSON.parse(await readFile('package.json', 'utf8')).version.replace(
  /\.0$/,
  '',
);
const langs = ['ja', 'en', 'zh', 'es', 'it', 'fr', 'la'] as const;
  ...
  async (t) => {
    ...
    for (const theme of ['googol-light', 'googol-night']) {
      await page.evaluate(async (theme) => {
        ...
        await Promise.all(
          [...document.querySelectorAll('#changelog img')].map((image) =>
    ...
    assert.equal(await page.locator('#changelog').count(), 0);
    await page.evaluate(() =>
      (document.querySelector('#blocking-dialog')! as HTMLDialogElement).close(),
    );
    await page.locator('#changelog article h1').waitFor();
    await page.locator('.changelog-close').click();
    await page.locator('#changelog').waitFor({ state: 'detached' });
