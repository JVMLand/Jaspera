...
test('Production static build works under a subdirectory and on mobile',{timeout:90000},async t=>{
 ...
 const server=createServer(async(req,res)=>{
   ...
   if(!pathname.startsWith('/playground/')){res.writeHead(404).end();return;}
   const path=resolve(root,pathname.slice('/playground/'.length)||'index.html');
   if(!path.startsWith(root)){res.writeHead(403).end();return;}
   try{const bytes=await readFile(path);res.writeHead(200,{'Content-Type':types[extname(path)]??'application/octet-stream'});res.end(bytes);}catch{res.writeHead(404).end();}
 });
 ...
 await mkdir('test-results',{recursive:true});await page.screenshot({path:'test-results/desktop.png'});
 ...
 for(const [id,bg] of [['japan-light','rgb(255, 255, 255)'],['japan-dark','rgb(20, 27, 41)'],['hitachi-light','rgb(255, 255, 255)'],['hitachi-dark','rgb(21, 29, 36)'],['denden-light','rgb(255, 255, 255)'],['denden-night','rgb(12, 25, 45)']]) {
  ...
  await page.locator('#theme-dialog button').click();await page.waitForTimeout(180);
 ...
 const pane=await page.locator('.source-pane').boundingBox();const popupEvent=page.waitForEvent('popup');
 await page.mouse.move(tab.x+tab.width/2,tab.y+tab.height/2);await page.mouse.down();await page.waitForTimeout(420);await page.mouse.move(pane.x+pane.width+40,pane.y+100,{steps:8});await page.mouse.up();
 const popup=await popupEvent;popup.on('pageerror',e=>errors.push(e.message));await popup.waitForFunction(()=>document.querySelector('#file-tabs [role=tab]')!==null);
 assert.match(popup.url(),/\/playground\/detached\.html/);assert.equal(await popup.locator('#file-tabs [aria-selected=true]').textContent(),'src/Main.jal');
 await popup.locator('#editor > .monaco-editor').click();await popup.keyboard.press('Control+a');await popup.keyboard.insertText('public class Main { bad }');
 await popup.locator('.squiggly-error').first().waitFor({timeout:20000});await popup.keyboard.press('Control+z');
 await popup.waitForFunction(()=>document.querySelector('.view-lines')?.textContent.replace(/\u00a0/g,' ').includes('Hello, World!'));

 await popup.close();await page.getByRole('tab',{name:'src/Main.jal',exact:true}).waitFor();
