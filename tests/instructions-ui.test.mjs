...
test('Instructions dictionary works without JVM and across themes',{timeout:90000},async t=>{
 ...
 const tab=page.locator('#instructions-tab'),panel=page.locator('#instructions-panel');await tab.click();assert.equal(await panel.isVisible(),true);assert.equal(await page.locator('#console-panel').isVisible(),false);await panel.locator('h2').getByText('iadd',{exact:true}).waitFor();assert.deepEqual(await panel.locator('.frame-value code').allTextContents(),['b : int','a : int','a + b : int']);
 ...
 await search.click();await category.selectOption('メソッド呼び出し・復帰');
 await search.fill('ireturn');await panel.locator('.instruction-index [data-op="ireturn"]').click();
 await panel.getByRole('navigation',{name:'関連命令'}).getByRole('link',{name:'areturn',exact:true}).click();
 assert.equal(await panel.locator('h2').textContent(),'areturn');
 assert.equal(await search.inputValue(),'');assert.equal(await category.inputValue(),'');
 assert.equal(await panel.locator('.instruction-chooser').isVisible(),false);
 assert.equal(await panel.locator('.instruction-detail').evaluate(e=>e===document.activeElement&&e.scrollTop===0),true);
 const returnLink=panel.getByRole('navigation',{name:'関連命令'}).getByRole('link',{name:'ireturn',exact:true});
 await returnLink.focus();await page.keyboard.press('Enter');assert.equal(await panel.locator('h2').textContent(),'ireturn');
 ...
 await search.fill('dup2_x2');await panel.locator('h2').getByText('dup2_x2',{exact:true}).waitFor();await page.locator('#instructions-tab').click();assert.equal(await panel.getByLabel('スタックの形式').count(),0);assert.equal(await panel.locator('.frame-transition').count(),4);assert.deepEqual(await panel.locator('.frame-transition').last().locator('.frame-value code').allTextContents(),['a','b','a','b','a']);assert.match(await panel.locator('.frame-note').last().textContent(),/カテゴリ2/);
 ...
 for(const theme of ['jal-night','darcula','japan-light','hitachi-light','hitachi-dark','denden-light','denden-night']){
