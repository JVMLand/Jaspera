...
test('size estimation neither serializes objects nor follows shared references twice; oversized arrays stop early', () => {
  const value: { text: string; self?: unknown; toJSON(): never } = {
    text: 'x'.repeat(500),
    toJSON() {
...
function documentsFixture() {
  ...
  const store = new SourceDocuments({
    ...
    setOrder: (v) => (order = v),
    create(path, source) {
      const model: { source: string; disposed?: boolean; getValue(): string; dispose(): void } = {
        source,
        getValue() {
          return this.source;
        },
...
test('closing preparation interrupts pending API and worker waits', async () => {
  const controller = new AbortController(),
    worker = Object.assign(new EventTarget(), { state: 'installing' });
