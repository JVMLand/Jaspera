...
const bundle = await build({
  entryPoints: ['src/parameter-slots.js'],
  bundle: true,
  write: false,
  platform: 'browser',
  format: 'esm',
});
const { parameterSlots } = await import(
...
test('parameters use local slots, category-2 values take two slots and arrays take one', () => {
  ...
  const hints = parameterSlots(source);
  assert.deepEqual(
    hints.map((h) => h.slot),
    [0, 1, 1, 3, 5, 6, 7, 8, 1],
  );
  assert.deepEqual(
    hints.map((h) => source[h.offset]),
    ['I', 'I', 'J', 'D', '[', '[', 'L', 'I', 'I'],
  );
  assert.deepEqual(
    hints.map((h) => h.width),
...
test('only declaration parameters are annotated, never calls, literals or comments', () => {
  const source = wrap(
    '/* public static fake(II)V {} */\npublic static empty()V { ldc "fake(II)V"\n invokestatic Main->calc(II)V\n return }\npublic static calc(I)V { invalid_body }',
  );
  assert.deepEqual(
    parameterSlots(source).map((h) => h.slot),
    [0],
  );
  assert.equal(
    source.slice(parameterSlots(source)[0].offset - 5, parameterSlots(source)[0].offset),
    'calc(',
  );
});
test('incomplete descriptors do not produce guessed hints; original offsets survive CRLF and Unicode', () => {
  ...
  assert.deepEqual(
    parameterSlots(source).map((h) => source.slice(h.offset, h.offset + 1)),
    ['I', 'L'],
  );
});
