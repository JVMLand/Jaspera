...
test(
  ...
  { timeout: 120000 },
  async (t) => {
    await mkdir('.cache/open-files', { recursive: true });
    await writeFile(
      '.cache/open-files/Opened.jal',
      `public class Opened (major_version=67, minor_version=0) {
 ...
 ireturn
    ...
    const bytecode = JSON.parse(compiled.stdout).bytecode;
    assert.ok(bytecode, compiled.stdout);
    const server = spawn(
      process.execPath,
      ['node_modules/vite/bin/vite.js', '--host', '127.0.0.1', '--port', '5211', '--strictPort'],
    ...
    const open = async (target, name, buffer) => {
      ...
      await target.locator('#open-files').click();
      await (
        await event
    ...
    await open(page, 'Helper.jal', 'public class Other {}');
    await page.getByRole('heading', { name: '同じ名前のファイルがあります' }).waitFor();
    await page.locator('#dialog-cancel').click();
    assert.equal(
      await page.evaluate(async () => (await import('/src/main.ts')).editor.getValue()),
      'public class Helper {}',
    );
    await open(page, 'Opened.CLASS', Buffer.from(bytecode, 'base64'));
    await page
      .getByRole('tab', { name: 'Opened.CLASS (JAL)', exact: true })
    ...
    await page.locator('#theme-dialog[open]').waitFor();
    await page.keyboard.press('Escape');
    ...
    await disk.addInitScript(() => {
      window.showDirectoryPicker = async () => {
        const root = await (
          await navigator.storage.getDirectory()
        ).getDirectoryHandle('open-project-test', { create: true });
        ...
        for (const [dir, name, text] of [
          ...
          [src, 'Main.jal', 'public class Main {}'],
        ] as [FileSystemDirectoryHandle, string, string][]) {
    ...
    await disk.goto(base);
    await open(
      disk,
      'demo.jalprj',
      JSON.stringify({ format: 'jalprj', version: 1, name: 'Disk', entryFile: 'src/Main.jal' }),
    );
    await disk.getByRole('button', { name: 'フォルダーを選ぶ', exact: true }).click();
    await disk.waitForFunction(
      () => document.querySelector('#project-name')?.textContent === 'Disk',
    );
    assert.equal(
      await disk.evaluate(async () => (await import('/src/main.ts')).editor.getValue()),
      'public class Main {}',
    );
    assert.equal(await disk.locator('dialog[open]').count(), 0);
