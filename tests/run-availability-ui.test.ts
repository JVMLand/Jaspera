...
import { launchBrowser, newAppPage, createTestProject, detachAt } from './helpers/browser.ts';
test(
  ...
  async (t) => {
    ...
    const enabled = async (value) =>
      page.waitForFunction(
        (value) => (document.querySelector('#run')! as HTMLButtonElement).disabled === !value,
        value,
      );
    ...
    for (const body of [
      ...
      await page.waitForFunction(() =>
        (document.querySelector('#run')! as HTMLElement).title.includes(
          'main メソッドがありません',
        ),
      );
      await page.waitForFunction(() =>
    ...
    await detachAt(
      page,
      (await page.getByRole('tab', { name: 'Main', exact: true }).boundingBox())!,
    );
    ...
    await popup.waitForFunction(
      () => (document.querySelector('#menu-run')! as HTMLButtonElement).disabled,
    );
