import test from 'node:test';import assert from 'node:assert/strict';import {build} from 'esbuild';import {readFile} from 'node:fs/promises';
const b=await build({entryPoints:['src/offsets.js'],bundle:true,write:false,platform:'browser',format:'esm'});
const {calculateOffsets}=await import('data:text/javascript;base64,'+Buffer.from(b.outputFiles[0].text).toString('base64'));
const wrap=body=>'public class Test { public static main([Ljava/lang/String;)V {\n'+body+'\n} }';
const offsets=body=>calculateOffsets(wrap(body)).map(i=>i.offset);
test('source offsets are independent of compilation and stack validity',()=>{assert.deepEqual(offsets('getstatic java/lang/System->out:Ljava/io/PrintStream;\nldc "Hello"\ninvokevirtual java/io/PrintStream->println(Ljava/lang/String;)V\nreturn'),[0,3,5,8]);assert.deepEqual(offsets('pop\nbipush 2\nsipush 300\nreturn'),[0,1,3,6]);});
test('wide and aligned variable-length switches follow Javasm sizes',()=>{assert.deepEqual(offsets('wide iload 256\nwide iinc 256 300\nreturn'),[0,4,10]);assert.deepEqual(offsets('iconst_0\ntableswitch 0 { a, b } default c\na: return\nb: return\nc: return'),[0,1,24,25,26]);assert.deepEqual(offsets('iconst_0\nlookupswitch { 1: a, 9: b, default: c }\na: return\nb: return\nc: return'),[0,1,28,29,30]);});
