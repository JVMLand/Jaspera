...
const bundle = await build({
  entryPoints: ['src/instruction-guide.ts'],
  bundle: true,
  write: false,
  platform: 'node',
  format: 'esm',
});
const { guide, instructionList, categories, category } = await import(
  'data:text/javascript;base64,' + Buffer.from(bundle.outputFiles[0].text).toString('base64')
);
test('every supported instruction has a category, explanation and diagram', () => {
  assert.equal(new Set(instructionList).size, 202);
  assert.ok(!instructionList.includes('aload_4'));
  for (const op of instructionList) {
    const e = guide(op);
    assert.ok(categories.includes(e.category), op);
    assert.ok(e.summary.length > 8, op);
    assert.ok(e.forms.length, op);
  }
  assert.deepEqual(
    instructionList.filter((op) => category(op) === '補助命令'),
    ['nop', 'wide'],
  );
});
test('small primitive values occupy int stack entries', () => {
  for (const op of ['i2b', 'i2c', 'i2s', 'baload', 'caload', 'saload'])
    assert.match(guide(op).forms[0].after[0], /: int$/);
  for (const op of ['bastore', 'castore', 'sastore'])
    assert.match(guide(op).forms[0].before.at(-1), /: int$/);
});
test('local diagrams describe only stores and increments', () => {
  assert.equal(guide('iload_1').forms[0].locals, undefined);
  assert.equal(guide('iadd').forms[0].locals, undefined);
  assert.ok(guide('istore_1').forms[0].locals);
  assert.deepEqual(guide('iinc').forms[0].before, []);
  assert.deepEqual(guide('iinc').forms[0].locals.after, ['#1: 4 : int']);
});
test('stack manipulation variants keep ordering and category restrictions', () => {
  const f = guide('dup2_x2').forms;
  assert.equal(f.length, 4);
  assert.deepEqual(f[3].before, ['b', 'a']);
  assert.deepEqual(f[3].after, ['a', 'b', 'a']);
  assert.match(f[3].note, /カテゴリ2/);
  assert.equal(guide('pop2').forms.length, 2);
...
test('explanations distinguish behaviour rather than inheriting generic documentation', () => {
  ...
  assert.equal(
    new Set(
      ['invokevirtual', 'invokeinterface', 'invokespecial', 'invokestatic', 'invokedynamic'].map(
        (op) => guide(op).summary,
      ),
    ).size,
    5,
...
test('related pages exist, exclude self, and keep other instruction behaviour out of the body', () => {
  for (const op of instructionList) {
    ...
    assert.ok(!related.includes(op), op);
    for (const target of related) assert.ok(instructionList.includes(target), `${op} -> ${target}`);
  }
