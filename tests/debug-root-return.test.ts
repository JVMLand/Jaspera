...
test(
  'step over the entry return finishes without opening runtime definitions',
  { timeout: 120000 },
  async (t) => {
    const base = 'http://127.0.0.1:5254';
    const server = spawn(
      process.execPath,
      ['node_modules/vite/bin/vite.js', '--host', '127.0.0.1', '--port', '5254', '--strictPort'],
    ...
    await page.waitForFunction(
      () => (document.querySelector('.debug-toolbar') as HTMLElement)?.dataset.state === 'paused',
    );
    ...
    await page.waitForFunction(() => {
      ...
      return (
        ...
        editor
          .getModel()!
          .getLineContent(Number(marker.getAttribute('data-line')))
          .trim() === 'return'
      );
    });
