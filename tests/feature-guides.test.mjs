...
test('feature guides follow their targets and remember dismissal across windows',{timeout:120000},async t=>{
 const base='http://127.0.0.1:5255';
 const server=spawn(process.execPath,['node_modules/vite/bin/vite.js','--host','127.0.0.1','--port','5255','--strictPort'],{stdio:'pipe',windowsHide:true});t.after(()=>server.kill());
 ...
 const context=await browser.newContext({viewport:{width:1500,height:1000}});const page=await context.newPage();page.setDefaultTimeout(60000);
 ...
 const rect=await guide.boundingBox();assert.ok(rect.x>=0&&rect.y>=0&&rect.x+rect.width<=720&&rect.y+rect.height<=700);
 await page.screenshot({path:'.cache/feature-guides-small.png'});
 await page.setViewportSize({width:1500,height:1000});await page.waitForTimeout(150);await page.screenshot({path:'.cache/feature-guides.png'});
 await page.locator('#menu-help').click();await page.locator('#help-shortcuts').click();await guide.waitFor({state:'hidden'});
 await page.locator('#help-dialog button').click();
 const popupEvent=page.waitForEvent('popup');await page.locator('#instructions-tab').click({button:'right'});await page.getByRole('menuitem',{name:'小窓で開く',exact:true}).click();
