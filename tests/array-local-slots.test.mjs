...
const directory='.cache/array-local-slots';await mkdir(directory,{recursive:true});
function java(args){const r=spawnSync('java',args,{encoding:'utf8'});assert.equal(r.status,0,r.stderr);return r.stdout;}
...
test('long and double arrays occupy one local slot, including multidimensional arrays and instance parameters',async()=>{
 const result=await compile('slots',`public class Slots (major_version=67, minor_version=0) {
 ...
 }`);assert.deepEqual(result.diagnostics,[]);assert.ok(result.bytecode);
 const probe=directory+'/Run.java';await writeFile(probe,`import java.util.*; public class Run extends ClassLoader {public static void main(String[] a) throws Exception {
...
test('the second slot of scalar long remains unreadable as an int',async()=>{
 const result=await compile('invalid','public class Invalid (major_version=67, minor_version=0) { public static value(JI)I { iload_1 ireturn } }');assert.equal(result.bytecode,'');assert.ok(result.diagnostics.some(d=>d.severity==='error'));
});
test('bundled ArrayList round-trips with all method frames and graphs',async()=>{
 const entry='java/util/ArrayList.class',bytes=unzipSync(await readFile('public/runtime/jdk23.jar'),{filter:f=>f.name===entry})[entry];assert.ok(bytes);
 ...
 const source=JSON.parse(java(['-cp','public/runtime/jalweb-compiler.jar',probe,Buffer.from(bytes).toString('base64')])).source;
 const result=await compile('ArrayList',source);assert.deepEqual(result.diagnostics,[]);assert.ok(result.bytecode);assert.equal(result.graphs.length,68);assert.equal(result.graphs.reduce((sum,g)=>sum+g.nodes.length,0),1533);assert.ok(result.stackFrames.length>0);
});
