...
test(
  ...
  async (t) => {
    const base = 'http://127.0.0.1:5214',
      server = spawn(
        process.execPath,
        ['node_modules/vite/bin/vite.js', '--host', '127.0.0.1', '--port', '5214', '--strictPort'],
    ...
    const labels = (target) => target.locator('.view-line span').filter({ hasText: /^\d+:$/ });
    await labels(page).first().waitFor();
    assert.deepEqual(await labels(page).allTextContents(), ['0:', '1:', '1:', '3:', '5:']);
    assert.equal(
    ...
    const style = await labels(page)
      ...
      .evaluate((n) => {
        ...
        return { radius: s.borderRadius, font: s.fontFamily, bg: s.backgroundColor };
      });
    assert.notEqual(style.radius, '0px');
    assert.match(style.font, /Segoe UI/);
    assert.match(style.bg, /rgba/);
      ...
      editor.executeEdits('test', [
        {
          range: {
            startLineNumber: 2,
            startColumn: line.indexOf('static') + 1,
            endLineNumber: 2,
            endColumn: line.indexOf('static') + 8,
          },
          text: '',
        },
      ]);
    ...
    await page.waitForFunction(
      () =>
        [...document.querySelectorAll('.view-line span')]
    ...
    await page.waitForFunction(
      () =>
        [...document.querySelectorAll('.view-line span')].filter((n) =>
    ...
    assert.deepEqual(await labels(popup).allTextContents(), ['0:', '1:', '1:', '3:', '5:']);
    assert.equal(
