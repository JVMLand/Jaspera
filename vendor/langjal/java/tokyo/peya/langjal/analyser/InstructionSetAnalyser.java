...
public class InstructionSetAnalyser {
    ...
    private FramePropagation[] analyseInstructionFrames() {
        ...
        for (InstructionInfo instruction : this.instructions) {
            ...
            try {
                frameDifference = instructionProducer.getFrameDifferenceInfo(instruction);
                this.processStackLocalDifference(instruction, frameDifference.getStackOperations());
            } catch (InstructionAnalysisException e) { throw e; }
            catch (ClassAnalyseException e) { throw new InstructionAnalysisException(instruction, -1, null, null, e); }
            ...
            switch (type) {
                ...
                case POP:
                    if (element instanceof LocalStackElement localElement)
                        ...
                        StackElement actual = this.stack.isEmpty() ? null : this.stack.peek();
